...
<br />
			      ...
			      <tr>
					...
					for($i=0;$i<count($accounts);$i++){
							...
							/* 正常销售订单 */
							$ss		= "SELECT SUM( ebay_total ) AS total, ebay_currency FROM ebay_order where (ebay_addtime>=$start1 and ebay_addtime<=$end1) and ebay_account='$accountsname' and ebay_status='2' and profitstatus='1' and (ebay_ordertype is null or ebay_ordertype= '-1') GROUP BY ebay_currency";
							...
							/* 计算总成本和总运费 */
							$ss		= "SELECT SUM( ordercopst ) AS ordercopst, SUM( ordershipfee ) AS ordershipfee FROM ebay_order where (ebay_addtime>=$start1 and ebay_addtime<=$end1) and ebay_account='$accountsname' and ebay_status='2'  and profitstatus='1'    ";
							$ss		= $dbcon->execute($ss);
							$ss		= $dbcon->getResultArray($ss);
