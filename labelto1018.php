...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'Buyer ID');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', 'shipping Address');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', '物品细节');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', '自己的id日期发货方式');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', '图片');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '单号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '物品链接');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', '售价USD');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', '进价RMB');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', '运费RMB');
	...
	for($i=0;$i<count($sql);$i++){
		...
		$ebay_note			= $sql[$i]['ebay_noteb'];
		$ebay_total	 		= @$sql[$i]['ebay_total'];
		$recordnumber		= $sql[$i]['recordnumber'];
		$ebay_account		= $sql[$i]['ebay_account'];
		$shiptime			= $sql[$i]['ebay_createdtime']?date('m.d',$sql[$i]['ebay_createdtime']):'';
		...
		$timeline			=  $ebay_account.'/'.$shiptime.'/'.$carrier;
		$ebay_currency		= $sql[$i]['ebay_currency'];
		$vv = "select rates from ebay_currency where user='$user' and currency='$ebay_currency'";
		$vv	= $dbcon->execute($vv);
		$vv	= $dbcon->getResultArray($vv);	
		if($vv){
			$totalprice					= $ebay_total*$vv[0]['rates'];
		}else{
		...
		$addressline	= $name.chr(10).$street1." ".$street2.chr(10).$city.", ".$state." ".$zip.chr(10).$countryname."/".$country[$countryname];
		...
		$objPHPExcel->setActiveSheetIndex(0)->getCell('A'.$a)->setValueExplicit($ebay_userid, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->getCell('B'.$a)->setValueExplicit($addressline, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->getCell('D'.$a)->setValueExplicit($timeline, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->getCell('F'.$a)->setValueExplicit($ebay_note, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->getCell('H'.$a)->setValueExplicit($totalprice, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		for($o=0;$o<count($sl);$o++){			
			...
			$strline		.= $sku.chr(10).$ebay_itemid." - Qty:".$amount." , Price ".$ebay_currency." {$ebay_signalfh}".$earchprice." each Shipfee:".$ebay_shipfee;
			$strline2		.= $amount."----".$sku.chr(10);
			
			
			
			
			$objPHPExcel->setActiveSheetIndex(0)->getCell('G'.$a)->setValueExplicit($attribute, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->setActiveSheetIndex(0)->getCell('C'.$a)->setValueExplicit($skuline, PHPExcel_Cell_DataType::TYPE_STRING);
		$a++;	
...
$objPHPExcel->getActiveSheet(0)->getStyle('A1:J500')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('A')->setWidth(9.5);	
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('B')->setWidth(54);	
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('C')->setWidth(19);
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('D')->setWidth(10);	
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('E')->setWidth(21);
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('F')->setWidth(14);
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('G')->setWidth(23);
...
$objPHPExcel->setActiveSheetIndex(0)->getStyle('B1:B500')->getFont()->setSize(14);
$styleArray1 = array(
    'font' => array(
       'bold' => true,
       'color'=>array(
...
$objPHPExcel->getActiveSheet(0)->getStyle('A1')->applyFromArray($styleArray1);
$objPHPExcel->getActiveSheet(0)->getStyle('B1:B500')->applyFromArray($styleArray1);
$objPHPExcel->getActiveSheet(0)->getStyle('H1')->applyFromArray($styleArray1);
$objPHPExcel->getActiveSheet(0)->getStyle('I1')->applyFromArray($styleArray1);
$objPHPExcel->getActiveSheet(0)->getStyle('J1')->applyFromArray($styleArray1);
$objPHPExcel->setActiveSheetIndex(0)->getRowDimension('A2:J500')->setRowHeight(75);
$title			= "Addresslist".date('Y-m-d');
$titlename		= "Addresslist".date('Y-m-d').".xls";
