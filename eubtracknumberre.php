...
<?PHP
	...
	function getshiplabel($ordersn){
		...
		$functions = $soapclient->RecreateAPACShippingPackage(array("RecreateAPACShippingPackageRequest"=>$params));
		...
		foreach($functions as $aa){
			...
			if($ack     != 'Failure'){
				...
				$dbcon->execute($sg);
				echo "<br>".$ebay_usermail." EUB订单重新发货成功";
			
			}else{
				$Message	= $bb['Message'];
				echo "<br>".$ebay_usermail." EUB订单重新发货失败，失败原因是：<font color='#FF0000'>".$Message."</font>";
