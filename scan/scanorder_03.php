<?php
	...
	$ss					= "select ebay_id from ebay_order where ebay_tracknumber='$value' and ebay_user ='$user' and scantime<=0 and (ebay_status='$allowauditorderstatus' or ebay_status='508') ";
	...
	if(count($ss)  == '0'){
		...
		$ss					= "select * from ebay_order where ebay_tracknumber='$value' and ebay_user ='$user' ";
		$ss					= $dbcon->execute($ss);
		$ss					= $dbcon->getResultArray($ss);
...
<script language="javascript">
	...
	function check01(){
			...
			if (keyCode == 13) {
				...
				location.href	= 'scanorder_03.php?value='+order+"&shiptype="+shiptype+"&tracknumber="+tracknumber+"&module=orders";
