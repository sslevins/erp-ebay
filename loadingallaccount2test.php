<?php
	...
	$vv				= "select distinct user  from ebay_user where user ='survy' or user ='guangpai' or user = 'vipchi' or user = 'vipzz' or user = 'vipqt' or user = 'viplinlin'  or user = 'vipgj' or user ='chineon ' ";
	$vv				= $dbcon->execute($vv);
	...
	for($j=0;$j<count($vv);$j++){
		...
		for($i=0;$i<count($sql);$i++){
			...
			$account	= $sql[$i]['ebay_account'];
				...
				$start		= date('Y-m-d H:i:s',strtotime("$cc -800 minutes"));
	...
	function GetOrderSiptype($user){
		...
		global $dbcon,$notesorderstatus;
					...
					$sql		= "select * from ebay_order as a where ebay_user='$user'  and ebay_combine!='1' and  ebay_status = '1' and (ebay_carrier = '' or ebay_carrier is null) ";
						...
						$sql		= "select * from ebay_order as a where ebay_id = '286999'  ";
						$sql	= $dbcon->execute($sql);
						$sql	= $dbcon->getResultArray($sql);
