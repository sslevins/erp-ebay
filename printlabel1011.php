...
<?php
	...
	if($_REQUEST['ordersn'] == ''){
		...
		$sql			= "select a.*,count(b.ebay_id) as cc  from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn  where  a.ebay_user='$user' ";
	...
	if($ostatus!=='100')
	 ...
	 if($Shipping!='')
			...
			if($hunhe=='2'){
				...
				$sql.=" group by a.ebay_id  order by  sku asc";
	}else{
		
		$sql			= "select a.*,count(b.ebay_id) as cc  from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn  where ($tj) and a.ebay_user='$user' group by a.ebay_id  order by  sku asc ";
	}
...
<table style="width:105mm" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>   
    ...
    <td>
    <table width="380" height="232px"   border="0" cellpadding="0" cellspacing="0"  >


      <tr>
        <td width="94%" height="35" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border:2px dashed #000000">
	 ...
	 echo "<strong>".$cname."</strong><br>";
		...
		echo '&nbsp;';
		 ...
		 ?>              &nbsp;</td>
            <td width="50%" valign="top" style="border-left:2px dashed #000000"><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" style="border-left:0px; border-right:0px; border-top:0px">
              <tr>
                <td><?php echo substr($ebayaccount,0,2).'***'.substr($ebayaccount,strlen($$ebayaccount)-2);?></td>
                <td><?php echo $ebay_id.'-'.$recordnumber;?></td>
              </tr>
              <?php
		...
		for($g=0;$g<count($ss);$g++){
			...
			echo "<font style='font-weight: bold;'>".$sku."  {$goods_location} ".$goods_name." X ".$ebay_amount."/</font><br>";			
          ...
          <tr>
            <td align="center"><div style="text-align:center; margin-bottom:3px; border-top:1px solid #000000"><img src="barcode128.class.php?data=<?php echo $ebay_id;?>" alt="" width="143" height="48" /></div></td>
            <td width="50%" valign="bottom" style="border-left:2px dashed #000000"><?php echo '<br>'.$ebay_noteb.' '.date('Y-m-d').'    '.($i+1).' / '.$totalpages;?></td>
          </tr>
  ...
  <td height="0px" ></td>
    ...
    <?php
	if(($i+1)%2 == 0 && $i !=0) echo "</tr><tr><td>&nbsp;</td></tr>";
	if(($i+1)%8 == 0 && $i !=0) echo '</tr></table><div style="PAGE-BREAK-AFTER: always">&nbsp;&nbsp;</div><table  style="width:105mm" border="0" cellpadding="0" cellspacing="0"><tr>';
