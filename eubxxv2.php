...
if($type=='del'){
	...
	function submiteub($ebay_id,$print){
		...
		foreach($sql as $k=>$v){
			...
			$value = str_replace('{rstreet}',$v['ebay_street'],$value);
			...
			foreach($vv as $kk=>$vvv){
				...
				if($ss){
					 ...
					 $value1 = str_replace('{weight}',$ss[0]['goods_weight'],$value1);
	...
	function printeub($id){
		...
		$sql = $dbcon->execute($sql);
		$sql = $dbcon->getResultArray($sql);
		$url =   'http://www.ems.com.cn/partner/api/public/p/print/batch';
		$header = array();
		$header[] = "Content-Type:text/xml; charset=utf-8";
		$header[] = "version:international_eub_us_1.1";
	...
	function deleub($id){
	    ...
	    $url =   "http://www.ems.com.cn/partner/api/public/p/order/".$ebay_tracknumber;
		$header = array();
		$header[] = "Content-Type:text/xml; charset=utf-8";
		$header[] = "version:international_eub_us_1.1";
		$rr				= "select id from ebay_account where ebay_account='$ebay_account'";
