<?php
	...
	$vv				= "select distinct user  from ebay_user where user ='survy' or user ='guangpai' or user = 'vipchi' or user = 'vipzz'  or user = 'vipqt'  or user = 'vipallen'  or user ='silent' or user = 'vipcz' or user = 'viphong' or user ='2013' or user = 'vip272' or user = 'vipddd' or user ='vipwx' or user ='viphcx' or user ='vipyx' or user ='viperp' or user='viphewei' or user ='vipfjadm' or user = 'shine_2012' or user ='vipnick' ";
	$vv				= $dbcon->execute($vv);
	...
	for($j=0;$j<count($vv);$j++){
		...
		for($i=0;$i<count($sql);$i++){
			...
			$account	= $sql[$i]['ebay_account'];
				...
				$start		= date('Y-m-d H:i:s',strtotime("$cc -660 minutes"));
		...
		global $dbcon,$notesorderstatus;
					...
					$sql		= "select * from ebay_order as a where ebay_user='$user' and  ebay_status = '1' and (ebay_carrier = '' or ebay_carrier is null) ";
						...
						for($i=0;$i<count($sql);$i++){
							...
							$totalweight			= 0;
							$goods_cost				= 0;
							$st	= "select sku,ebay_amount,ebay_id , ebay_shiptype from ebay_orderdetail where ebay_ordersn='$ebay_ordersn'";
							$st = $dbcon->execute($st);
