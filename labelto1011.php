...
JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn  where  a.ebay_user='$user' and a.ebay_status = '$ostatus' and a.ebay_combine!='1'  ORDER BY b.sku  ";		
	}else{	
...
JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn where ($ertj) and a.ebay_user='$user' and a.ebay_combine!='1' ORDER BY b.sku";	
	...
	$objPHPExcel->setActiveSheetIndex(0)->getCell('A1')->setValueExplicit('序号', PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->setActiveSheetIndex(0)->getCell('B1')->setValueExplicit('ID', PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->setActiveSheetIndex(0)->getCell('C1')->setValueExplicit('地址', PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->setActiveSheetIndex(0)->getCell('D1')->setValueExplicit('物品', PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->setActiveSheetIndex(0)->getCell('E1')->setValueExplicit('备注', PHPExcel_Cell_DataType::TYPE_STRING);
	
	$objPHPExcel->setActiveSheetIndex(0)->getCell('F1')->setValueExplicit('Paypal Email', PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->setActiveSheetIndex(0)->getCell('G1')->setValueExplicit('订单编号', PHPExcel_Cell_DataType::TYPE_STRING);
...
$Custom_Paypalemailaddress		= $sql[$i]['Custom_Paypalemailaddress'];
		$ebay_account		= $sql[$i]['ebay_account'];
		
		if($tel == ''){
				
			$addressline	= $name.chr(10).$street1." ".$street2.chr(10).$city.", ".$state.chr(10).$zip.chr(10).$countryname;
		...
		$ss	= "select sku,ebay_amount from ebay_orderdetail as a where    a.ebay_ordersn = '$ordersn'  ";
		$ss		= $dbcon->execute($ss);
		$ss		= $dbcon->getResultArray($ss);
		$skustr="";
		for($j=0;$j<count($ss);$j++){
			$skustr.=$ss[$j]['sku'].' * '.$ss[$j]['ebay_amount'].";  ";
		}
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'.$a.':A'.($a+1));
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.$a, ($i+1), PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('B'.$a.':B'.($a+1));
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$a, $ebay_userid, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('A'.($a+1))->setValueExplicit($addressline, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a, 'TO:', PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.($a+1), $addressline, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getRowDimension($a+1)->setRowHeight();
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('D'.$a.':D'.($a+1));
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$a, $skustr, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('E'.$a.':E'.($a+1));
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$a, "", PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$a, $Custom_Paypalemailaddress, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$a, $ebay_id, PHPExcel_Cell_DataType::TYPE_STRING);
...
$objPHPExcel->getActiveSheet()->getStyle('A1:F500')->getFont()->setSize(13);

$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('C')->setWidth(35.5);	
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('D')->setWidth(25.5);


$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('E')->setWidth(35.5);	
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('F')->setWidth(25.5);





$title			= "Delivery_Record".date('Y-m-d');
$titlename		= "Delivery_Record".date('Y-m-d').".xls";
