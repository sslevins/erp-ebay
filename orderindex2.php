...
<input type="button" value="标打印"		 	onclick="ordermarkprint('add')" />
<input type="button" value="取消打印"		 	onclick="ordermarkprint('del')" />
          ...
          <input type="button" value="标发出" onclick="ebaymarket(0)" />
          <input type="button" value="Amazon 批量传跟踪号" onclick="ammarket(0)" />
         
          <input type="button" value="开始配货" onclick="distribute03()" />
          <select name="filesid" id="filesid" onchange="exportstofiles()"  style="width:90px">
              <option value="" id="filesidselect">导出到XLS</option>
               <option value="12">热敏EUB-xls</option>
                   ...
                   <option value="5">拣货汇总单_01-xls</option>
                   
                    <option value="104">拣货汇总单_02-xls</option>
                     <option value="107">拣货汇总单_03_无库存-xls</option>
				   ...
				   <option value="20">SalesHistory-csv</option>
                   <option value="112">一键导出无库存商品</option>
				   <option value="c1">仓库1</option>
				   <option value="c2">仓库2</option>
				   <option value="c3">仓库3</option>
                   <option value="c4">发货清单_UK</option>
                   
                    <option value="999">SalesHistory-csv-汇总导出</option>
          ...
          <select name="printid" id="printid" onchange="printtofiles()"  style="width:90px" >
                   <option value="" id="printidselect">打印订单</option>
                    <option value="16" >Packing Slip打印</option>
                    <option value="248" >Invoice-打印</option>
                    <option value="18" >夏普格式打印</option>
					<option value="181" >夏普格式打印（修）</option>
                   <option value="15" >自订(3)A4包货清单打印</option>
                   <option value="14" >自订(2)A4包货清单打印</option>
                   <option value="13" >自订(1)标签打印-40*60</option>
				   ...
				   <option value="241" >地址+物品+图片每页5个-A4打印</option>
				   <option value="40">地址+条码+物品每页5个-A4打印(修)</option>
                   <option value="41">地址+条码+物品每页5个-A4打印(修2)</option>
                   <option value="7" >地址每页10个-A4打印</option>
                   <option value="11" >地址每页10个+条码+sku-A4打印</option>
                   <option value="42" >地址每页10个+条码+sku-A4打印(修改)</option>
                   <option value="9" >地址每页8个+条码+sku-A4打印</option>
                   ...
                   <option value="246" >THG lable 102mm*70mm打印</option>
   ...
   <input type="button" value="SMT导入" onclick="location.href='ordersumaitongupload.php?module=orders&amp;type=addorder&amp;action=速买通导入模板'" />
                 <input type="button" value="Am导入" onclick="location.href='orderukupload.php?module=orders&amp;type=addorder&amp;action=Amazon'" />
                 <input type="button" value="跟踪号批量" onclick="location.href='orderuploadloader.php?module=orders&amp;type=addorder&amp;action=Amazon'" />
		  ...
		  <input type="button" value="eBay Csv" onclick="location.href='salehistoryupload.php?module=orders&amp;type=addorder&amp;action=ebay csv导入'" />
          
          <?php if(in_array("orders_warehfffffffouse",$cpower)){?>
          <input type="button" value="分配库存" onclick="distribute()" />
          <?php }?>
...
<input type="button" value="HK热敏" onclick="eubtracknumberv3rmHK(1)" />
<input type="button" value="HK A4" onclick="eubtracknumberv3rmHK(0)" />
<br />
          <div style="border-bottom:#CCCCCC solid 1px">&nbsp;</div>
 4px :
...
<br />
          ...
          <select name="Shipping" id="Shipping" style="width:90px" >
		   ...
		   $gg	= "select count(1) as cc from ebay_order as a where a.ebay_user='$user'  and a.ebay_combine!='1'   and ($ebayacc) and  ebay_carrier ='$name'";
		   
		   }else{
		   
		   $gg	= "select count(1) as cc from ebay_order as a where a.ebay_user='$user'  and a.ebay_combine!='1'   and ($ebayacc) and a.ebay_status='$ostatus' and ebay_carrier ='$name'";
		   }
         ...
         <select name="country" id="country" onchange="changeaccount()">
              ...
              <?php 
					...
					$sql	 = "select distinct ebay_countryname from ebay_order where ebay_user='$user' order by ebay_countryname desc ";
					$sql	 = $dbcon->execute($sql);
					$sql	 = $dbcon->getResultArray($sql);
					for($i=0;$i<count($sql);$i++){					
					 	$ebay_countryname	= $sql[$i]['ebay_countryname'];
					 ?>
              <option value="<?php echo $ebay_countryname;?>" <?php if($country == $ebay_countryname ) echo 'selected="selected"';?> ><?php echo $ebay_countryname;?></option>
              <?php } ?>
            </select>
...
//stockstatus
			  ...
			  ?>
         		...
         		<tr height='20' class='oddListRowS1'>
						...
						<td scope='row' align='left' valign="top" ><a href="ordermodifive.php?ordersn=<?php echo $ordersn;?>&module=orders&ostatus=1&action=Modifive Order"  target="_blank">编辑</a>&nbsp;
...
<td scope='row' align='left' valign="top" ><?php 
			...
			// echo"	  交昜号：".$ebay_ptid ."  Paypal:".$PayPalEmailAddress;
				 ...
				 <?php //} ?>
                  ...
                  <a href="#" onclick="rta('<?php echo $ebayid;?>','<?php echo $pid;?>')">RMA 管理<?php echo $ebay_shiptype;?></td></a> 
					...
					for($e=0;$e<count($goods_sncombine);$e++){
                      ...
                      <tr>
						...
						$dd				= "select sum(b.goods_count) as cc,isuse from ebay_goods as a join  ebay_onhandle as b on a.goods_id = b.goods_id  where a.goods_sn ='$goods_sn' and store_id != '' and a.ebay_user ='$user' ";
