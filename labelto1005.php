...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '图片');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', 'SKU');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', '产品名称(中文)');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', '数量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '平台配货数量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '配货日期');
...
where a.ebay_user='$user'   and a.ebay_combine!='1' and ($ertj) GROUP BY b.sku ";	
	...
	for($i=0;$i<count($sql);$i++){
		...
		$sku					= $sql[$i]['sku'];	
			...
			if(count($rr) > 0){
				$goods_name = $rr[0]['notes'];
									...
									$goods_sncombine    = explode(',',$goods_sncombine);	
									 for($e=0;$e<count($goods_sncombine);$e++){
											 ...
											 $goods_name = $ee[0]['goods_name'].' '.$goods_name;
											 $vv				= "select * from ebay_labeltotable where goods_sn ='$goods_sn'  and ebay_user='$user' ";
											 $vv				= $dbcon->execute($vv);
											 $vv 		 	= $dbcon->getResultArray($vv);
											 if(count($vv) >0){
											 $vv			= "update ebay_labeltotable set goods_amount = goods_amount + $goddscount where goods_sn ='$goods_sn' ";
											 }else{
											$vv			= "insert into ebay_labeltotable (goods_sn,goods_name,goods_amount,ebay_user) values('$goods_sn','$goods_name','$goddscount','$user') ";
											 }
										 $dbcon->execute($vv);
				...
				$vv 		 	= $dbcon->getResultArray($vv);
				if(count($vv) >0){
						$vv			= "update ebay_labeltotable set goods_amount = goods_amount + $goddscount where goods_sn ='$sku'  and ebay_user='$user' ";
				}else{
						$vv			= "insert into ebay_labeltotable (goods_sn,goods_name,goods_amount,ebay_user) values('$sku','$goods_name','$amount','$user') ";
				}
	...
	$vv				= "select * from ebay_labeltotable where ebay_user ='$user' order by goods_sn asc  ";
	$vv				= $dbcon->execute($vv);
	$vv 		 	= $dbcon->getResultArray($vv);
	...
	for($i=0;$i<count($vv);$i++){
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a, $sku.chr(13).chr(13).chr(13), PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$a, $goods_name, PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$a, $goods_amount, PHPExcel_Cell_DataType::TYPE_STRING);
				
				
				if($user == 'vipyisi') $pic = $sku.'.jpg';
...
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('D')->setWidth(23);	
