		...
		<td width="65%">
			<table border='0' cellpadding='0' cellspacing='0' width='100%' class='paginationTable'>
             ...
             <?php
						...
						$end		= date('Y-m-d',strtotime("$cc +1 days"))."T23:59:59";
						...
						$sql 		 = "select * from ebay_paypal where user='vipliang'";
						$sql		 = $dbcon->execute($sql);
						$sql		 = $dbcon->getResultArray($sql);
						for($i=0;$i<count($sql);$i++){
						
							$account		 = $sql[$i]['account'];	
							$start		= date('Y-m-d',strtotime("$cc -1 days"));						
							$end		= date('Y-m-d');						
							getPaypalsales($start,$end,$account);
