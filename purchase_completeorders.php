  ...
  <?php
      ...
      <select name="factory" id="factory">
        ...
        <?php 
							...
							?>
        <option value="<?php echo $company_name;?>" <?php if($company_name ==$factory) echo "selected=selected";?>><?php echo $company_name; ?></option>
        <?php
							}
							?>
      </select>
...
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view' id='list1'>
   ...
   <?php 
				  	...
				  	$sql = "select * from  ebay_iostore as a  where a.ebay_user='$user' and a.type='94' and a.io_status='0'";	
					...
					if($factory != '') $sql .=" and io_partner='$factory'";
...
<?php if($billtype == '1' ){ ?>
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view' id='list2'>
	<tr class='pagination'>
		<td colspan='13'>&nbsp;			</td>
	</tr><tr height='20'>
					<th scope='col' nowrap="nowrap">
   ...
   <?php 
			      ...
			      $sql = "select b.goods_sn,b.goods_name,b.goods_unit,sum(goods_count) as allcount from  ebay_iostore as a join ebay_iostoredetail as b on a.io_ordersn = b.io_ordersn where a.ebay_user='$user' and a.type='94' and a.io_status='0' ";	
				...
				$sql = $sql.$limit;
					...
					for($i=0;$i<count($sql);$i++){
						...
						$vv = "select goods_cost from  ebay_goods where ebay_user='$user' and goods_sn='$sku'";
						$vv = $dbcon->execute($vv);
						$vv = $dbcon->getResultArray($vv);
				  ...
				  ?>
					<tr height='20' class='oddListRowS1'>
							...
							<td scope='row' align='left' valign="top" ><?php echo $dataarray[4];?>&nbsp;</td>
...
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view' id='list3'  >
   ...
   <?php 
					...
					$sql = "select * from  ebay_iostore as a join ebay_iostoredetail as b on a.io_ordersn = b.io_ordersn where a.ebay_user='$user' and a.type='94' and a.io_status='0' ";	
					if($keys != '') $sql .= " and b.goods_sn='$keys'";
					if($startdate != '' && $startdate != '') {
						$start		= strtotime($startdate.' 00:00:00');
						$end		= strtotime($startdate.' 23:59:59');
				...
				$sql = $sql.$limit;
					...
					for($i=0;$i<count($sql);$i++){
						...
						$vv = "select goods_cost from  ebay_goods where ebay_user='$user' and goods_sn='$goods_sn'";
						$vv = $dbcon->execute($vv);
						$vv = $dbcon->getResultArray($vv);
						$lastcost = $vv[0]['goods_cost'];
...
<script language="javascript">
	...
	function searchorder(){
		...
		location.href	= 'purchase_completeorders.php?keys='+keys+"&module=purchase&action=&stype=<?php echo $stype; ?>&dstatus=<?php echo $dstatus; ?>&warehouse="+warehouse+"&startdate="+startdate+"&enddate="+enddate+"&billtype="+billtype+"&factory="+encodeURIComponent(factory);
