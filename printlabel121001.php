...
$tj = '';
 ...
 ?>
	<td <?php if($line>0) echo 'rowspan="2"' ;?> style="width:50mm;min-height:38mm;border:1px solid #000000;padding:2px;font-size:12px; margin:1px;">
		<div style="width:189px;line-height:20px;">
		<ul>
								...
								if(count($rr) > 0){
									...
									for($e=0;$e<count($goods_sncombine);$e++){
											...
											if($goods_sn != ''){
													...
													if($notes){
														$skuline = $goods_sn.'*'.$goddscount.'('.$notes.'),';
													}else{
														$skuline = $goods_sn.'*'.$goddscount.',';
													}
													$zzz++;
													echo $skuline;
													if(($zzz)%2 ==0 || strlen($skuline)>14) echo '<br>';
											}
										...
										if($sss[0]['goods_note']){
											$skuline = $sku.'*'.$amount.'('.$sss[0]['goods_note'].'),';
										}else{
											$skuline = $sku.'*'.$amount.',';
										}
										$zzz++;
										echo $skuline;
										if((($zzz)%2 ==0 && ($k+1)!= count($ss)) || strlen($skuline)>14) echo '<br>';
