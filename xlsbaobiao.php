...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '仓库扫描时间');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '运费');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', '邮递方式');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', '跟踪号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'sku * 数量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '记录数');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '重量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', '国家');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', '系统订单编号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', 'eBay 帐号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', '扫描人');
	...
	$sql	= "select * from ebay_order as a where ebay_user='$user'  and a.ebay_combine!='1' and (a.scantime >=$start and a.scantime <=$end) ";	
	if($ebay_carrier != '') $sql .= " and a.ebay_carrier='$ebay_carrier' ";
	if($tjstr != '' ) $sql .=  " and ($tjstr) ";
	...
	for($i=0;$i<count($sql);$i++){
		...
		$id						= $ss[0]['id'];
		$ordershipfee			= number_format(shipfeecalc($id,$totalweight,$countryname),2);
		...
		for($j=0;$j<count($sl);$j++){
			$skustr		.= $sl[$j]['sku'].'*'.$sl[$j]['ebay_amount'];
		}						

		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$a, $scantime);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$a, $ordershipfee);
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$a, $skustr);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$a, $j);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$a, $orderweight2);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$a, $countryname.'-'.$cnname);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$a, $ebay_id);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$a, $ebay_account);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$a, $packinguser);
		$a++;
...
$objPHPExcel->setActiveSheetIndex(0)->getColumnDimension('G')->setWidth(3);
