...
class Struct extends  Configuration {
	...
	private static $isMulArray = false;
	...
	public static function preAlertOrderService($customerParameter) {//用php基类生成一个结构体【定义参数规则】
		$temp = array(
				'in0' => self::AUTHTOKEN,
				'in1' => $customerParameter//self::returnStructArr($customerParameter)
		);
		self::$isMulArray = true;
		return $temp;
	}
	
	
	//创建并预报订单
	public static function createAndPreAlertOrderService($customerParameter,$token) {//用php基类生成一个结构体【定义参数规则】
		//参数模
		$declareInvoice = array(
				"declareNote" => '',
				"declarePieces" => '',
		...
		$temp = array(
				'in0' => $token,
				'in1' => $createAndPreAlertOrderArray
		);
		//print_r($temp);
		return $temp;
	}
