...
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view'>
  ...
  <tr>
    ...
    <th bgcolor="#eeeeee" class="left_txt">Reply Time</th>
		...
		$cidtypes=$cidtype;			//				
	$cidtype 	= explode(",",$cidtype );
$classid= " and  (  ";
 for($o=0;$o<count($cidtype);$o++){
 			$cid	= $cidtype[$o];
			if ($o>0){$classid= $classid ."	or classid='$cid' ";}else{
		$classid= $classid ."	 classid='$cid' ";}
}
		$classid= $classid ."	) ";
					$sql = "select replytime,sendid,subject,createtime,status,recipientid,itemid,title,message_id,classid,title,classid,replyuser,mmarket,id
	 from ebay_message where     ebay_user='$user' and status ='$os'    ".$classid;
 				/*	$sql = "select replytime,sendid,subject,createtime,status,recipientid,itemid,title,message_id,classid,title,classid,replyuser,mmarket,id
	 from ebay_message where classid='$cidtypes' and ebay_user='$user' and status ='$os'";
	 
	 */
				}else{

					//$sql = "select * from ebay_message where forms='$os' and ebay_user='$user' and (classid = '0' or classid = '')";
					$sql = "select replytime,sendid,subject,createtime,status,recipientid,itemid,title,message_id,classid,title,classid,replyuser,mmarket,id from ebay_message where forms='$os' and ebay_user='$user' ";

				}
				
				if($os == 'all'){
				
					$sql = "select replytime,sendid,subject,createtime,status,recipientid,itemid,title,message_id,classid,title,classid,replyuser,mmarket,id from ebay_message where ebay_user='$user' ";
...
function marketbwh(){
	...
	function searchs(){
		...
		location.href= "messageindex.php?module=message&type=class&ostatus=<?php echo $os ?>&keys="+keys+"&searchs="+searchs+"&ebay_account="+ebay_account+"&action=<?php echo $_REQUEST['action'];?>&replystatus="+replystatus+"&cidtype=<?php echo $_REQUEST['cidtype'];?>";
