...
for($i=1;$i<count($goods_list);$i++){
	 //订单号
	$recordnumber				= $goods_list[$i][1];
	if($recordnumber != ''){
	
	
	$ebay_ordersn				=$user.$recordnumber;
	$ebay_userid				= $goods_list[$i][33]; //买家名称
	$ebay_username				= $goods_list[$i][18]; //买家名称
	$ebay_createdtime			= $goods_list[$i][6]; // 下单时间
	$ebay_paidtime				= $goods_list[$i][7]; // 付款时间
	$ebay_paidtime					= strtotime($ebay_paidtime);
	$ebay_createdtime				= strtotime($ebay_createdtime);
	
	
	$ebay_itemprice				= $goods_list[$i][32];   //产品总金额
	$ebay_shipfee				= 0;   // 物流费用
	$ebay_total					= $goods_list[$i][32];   //订单金额


	$ebay_note					= $goods_list[$i][35];	//订单备注
	//$ebay_noteb					= str_replace("'",'',$goods_list[$i][23]);	//订单备注
	$ebay_currency				= $goods_list[$i][27];	//订单备注
	$ebay_countryname			= $goods_list[$i][24];
	$ebay_state					= '';
	$ebay_city					= '';
	$ebay_street				= $goods_list[$i][22];
	$ebay_postcode				= $goods_list[$i][23];
	$ebay_phone					= $goods_list[$i][20].$goods_list[$i][21];
	// if($ebay_phone){
		// $ebay_phone					= substr($ebay_phone,1);
	// }
	$ebay_carrier				= $goods_list[$i][3];   //买家选择物流
	$sku						= $goods_list[$i][16];
	$title				= $goods_list[$i][13];
	$ebay_itemid		= $goods_list[$i][12];
	$amount		= $goods_list[$i][14];
	...
	if($vv == '0'){
		...
		$sql					= "insert into  ebay_order(ebay_ordersn,recordnumber,ebay_createdtime,ebay_paidtime,ebay_userid,ebay_username,ebay_street,ebay_street1,ebay_city,ebay_state,ebay_countryname,ebay_postcode,ebay_phone,ebay_total,ebay_status,ebay_user,ebay_addtime,ebay_shipfee,ebay_account,ebay_note,ebay_carrier,ebay_warehouse,ebay_currency) values('$ebay_ordersn','$recordnumber','$ebay_createdtime','$ebay_paidtime','$ebay_userid','$ebay_username','$ebay_street','$ebay_street1','$ebay_city','$ebay_state','$ebay_countryname','$ebay_postcode','$ebay_phone','$ebay_total','$orderstatus','$user','$mctime','$ebay_shipfee','$account','$ebay_note','$ebay_carrier','$defaultstoreid','$ebay_currency')";
		...
		if($sql == 0){
				...
				$esql    .= "`ebay_amount` ,`ebay_user`,`sku`,`shipingfee`,`ebay_account`,`addtime`,`recordnumber`,`goods_location`)VALUES ('$ebay_ordersn', '$ebay_itemid' ,'$title' , '$ebay_itemprice' , '$amount'";
