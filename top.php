...
<?php if($user == 'survy' || $user == 'vip778' || $user == 'silent' || $user == 'viperp' || $user == 'vipbin'  || $user == 'vpcxbusps' || $user == 'vipfangni' || $user == 'vipwx' || $user == 'sz108' || $user == 'yml' || $user == 'CAN' || $user == 'vipgavin'   ){ ?>
<li class="notCurrentTabLeft"> 
<span class="notCurrentTabLeft">&nbsp;</span><span class="<?php if($module == 'dispute') echo 'currentTab'; else echo 'notCurrentTab'; ?>"><a href="dispute_index.php?module=dispute&action=售后纠纷&mstatus=0" id="moduleTab_Opportunities">售后纠纷</a></span><span class="notCurrentTabLeft">&nbsp;</span> 
</li>
<?php } ?>
...
<?php if($_SESSION['user'] == 'survy' || $_SESSION['user'] == 'viphong' || $_SESSION['user'] == 'yy2894' || $_SESSION['user'] == 'vip778' || $user == 'vipbin' || $user == 'vpcxbusps' || $user == 'vipnick' || $user == 'silent'  || $user == 'vipfangni' || $user == 'yml' || $user == 'CAN' || $user == 'vipgavin' ){ ?>
...
<?php } ?>
         ...
         <li class="notCurrentTabLeft"> 
        <span class="notCurrentTabLeft">&nbsp;</span><span class="<?php if($module == 'advance') echo 'currentTab'; else echo 'notCurrentTab'; ?>"><a href="advance_index.php?module=advance&action=" id="moduleTab_Opportunities">广告管理</a></span><span class="notCurrentTabLeft">&nbsp;</span> 
        </li>
...
<span style="white-space:nowrap;"><a href="feedbackitbb.php?module=feedback&action=itemnumber报表">&nbsp;<span>itemnumber报表</span></a>
<?php } ?>
<!-- 订单管理 -->
<?php if($module == 'case'){ ?>
...
<span style="white-space:nowrap;"><a href="orderindex.php?module=orders&ostatus=0&action=未付款">&nbsp;未付款</a>


<span style="white-space:nowrap;"><a href="orderindex.php?module=orders&ostatus=1&action=待处理">&nbsp;<span>待处理(<font color="#FF0000">
<?php
...
$sql	= "select count(ebay_id) as cc from ebay_order as a where a.ebay_status='1' and a.ebay_user='$user'  ";
...
for($i=0;$i<count($ss);$i++){
	$ssid		= $ss[$i]['id'];
	$ssname		= $ss[$i]['name'];
	if(($i+1) % 10  == 0) echo '<br><br><b style="white-space:nowrap;">Actions:&nbsp;&nbsp;</b> ';
?>

<span style="white-space:nowrap;"><a href="orderindex.php?module=orders&ostatus=<?php echo $ssid;?>&action=<?php echo $ssname;?>&sortstatus=0">&nbsp;<span><?php echo $ssname;?>(<font color="#FF0000">
<?php
      //  if($ssname=='缺货'){
        	//$sql="select goods_count from ebay_onhandle where ";
       // }
$sql	= "select count(ebay_id) as cc from ebay_order as a where a.ebay_status='$ssid'   ";
$sqla	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sqla);
echo $sql[0]['cc'];
?>
...
<span style="white-space:nowrap;"><a href="orderindex.php?module=orders&ostatus=2&action=已经发货">&nbsp;<span>已经发货</span></a>

<span style="white-space:nowrap;"><a href="scanorder_baoiao.php?module=orders&action=Loading Orders">订单扫描</a></span>
<span style="white-space:nowrap;"><a href="orderloadtj.php?module=orders&ostatus=1&action=数据统计">报表</a></span> 
<span style="white-space:nowrap;"><a href="orderload.php?module=orders&action=同步订单">eBay同步</a></span>
<span style="white-space:nowrap;"><a href="orderloadsmt.php?module=orders&action=速卖通同步">速卖通同步</a></span>
<span style="white-space:nowrap;"><a href="amazonorderload.php?module=orders&action=同步订单">Amazon同步</a></span> 
<span style="white-space:nowrap;"><a href="magentosysorders.php?module=orders&action=同步订单">Magento线下同步</a></span>
<span style="white-space:nowrap;"><a href="paypalorerload.php?module=orders&action=同步订单">Paypal线下同步</a></span>
...
<?php
$sql		= "select ebay_id from ebay_order as a where ebay_user='$user' and ebay_status='2' and (mailstatus='0' or mailstatus ='') ";
...
(<font color="#FF0000">
<?php
$sql		= "select ebay_id from ebay_order as a where ebay_user='$user' and mailstatus='1' ";
$query		= $dbcon->query($sql);
$total		= $dbcon->num_rows($query);
echo $total;
...
(<font color="#FF0000">
<?php
$sql		= "select ebay_id from ebay_order as a where ebay_user='$user' and   and mailstatus='2'  ";
$query		= $dbcon->query($sql);
$total		= $dbcon->num_rows($query);
echo $total;
/* 释放mysql 系统资源 */
...
<span style="white-space:nowrap;"><a href="messageindex.php?module=message&action=所有Message&ostatus=all">&nbsp;<span>All Message</span></a> 
<span style="white-space:nowrap;"><a href="messageindex.php?module=message&action=From Message&ostatus=0">&nbsp;<span>From members</span></a> 


<span style="white-space:nowrap;"><a href="messageindex.php?module=message&action=From eBay&ostatus=2">&nbsp;<span>From eBay</span></a> 


<span style="white-space:nowrap;"><a href="messageindex.php?module=message&action=Hight priority&ostatus=3">&nbsp;<span>High priority</span></a> 
...
(<font color="#FF0000"><?php
$sql	= "select count(ebay_id) as cc from ebay_order where ebay_user='$user' and ebay_status='2' and profitstatus=0 ";
$sql	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sql);
echo $sql[0]['cc'];
?></font>)
...
(<font color="#FF0000"><?php
$sql	= "select count(ebay_id) as cc from ebay_order where ebay_user='$user' and profitstatus=1 ";
$sql	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sql);
echo $sql[0]['cc'];
?></font>)
...
(<font color="#FF0000">
<?php
$sql	= "select count(id) as cc from ebay_list where status='0' and ebay_user='$user' ";
$sql	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sql);
echo $sql[0]['cc'];
?>
...
(<font color="#FF0000">
<?php
$sql	= "select count(id) as cc from ebay_list where status='1' and ebay_user='$user' ";
$sql	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sql);
echo $sql[0]['cc'];
?>
...
<span style="white-space:nowrap;"><a href="systemmagento.php?module=system&action=Magento帐号管理">&nbsp;<span>Magento帐号管理</span></a> 
...
<?php if($module == 'dispute'){ ?>
<span style="white-space:nowrap;"><a href="dispute_index.php?module=dispute&action=售后纠纷&mstatus=0">&nbsp;<span>售后纠纷</span></a>
<span style="white-space:nowrap;"><a href="dispute_import.php?module=dispute&action=售后纠纷">&nbsp;<span>同步纠纷</span></a>
<?php } ?>
