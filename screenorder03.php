...
include "include/config.php";
	...
	if($_POST['submit']){
		...
		$vvsql		="select ebay_id,ebay_ordersn,ebay_warehouse from ebay_order as a where ebay_user = '$user' and ebay_status='$distrubitestock'  and ($ebayacc)";	
		$tjstr		= '';
		if($bill != ''){
	...
	function GetStockedUsedSKU01($orderstatus,$goods_sn,$storeid){
			global $dbcon,$user;
			/* 检查此sku是否是组合产品 */
				$gsql			= "SELECT sum(b.ebay_amount) as qty FROM ebay_order AS a JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn WHERE sku =  '$goods_sn' and  b.istrue = '0' and a.ebay_warehouse = '$storeid' and a.ebay_status='$orderstatus'";
				...
				for($i=0;$i<count($vv);$i++){
					...
					for($j=0; $j<count($fxgoods_sncombine);$j++){
						...
						if(strstr($fxlaberstr,$goods_sn)){
							...
							$gsql			= "SELECT sum(b.ebay_amount) as qty FROM ebay_order AS a JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn WHERE sku =  '$cgoods_sn' and  b.istrue = '0' and a.ebay_warehouse = '$storeid'   and a.ebay_status='$orderstatus' ";
							$gsql			= $dbcon->execute($gsql);
							$gsql			= $dbcon->getResultArray($gsql);
