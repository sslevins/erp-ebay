   ...
   <option value="130423">发货列表-flyta_v7_1</option>
          ...
          <select name="orderoperation" id="orderoperation"    onchange="orderoperation()" >
            ...
            <option value="9">批量修改</option>
              ...
              <option value="">-----EUB v2.0-----</option>
              <option value="63">EUB-申请跟踪号</option>
              <option value="64">EUB-交运订单</option>
              <option value="65">EUB-热敏打印</option>
              <option value="66">EUB-A4打印</option>
              <option value="67">EUB取消跟踪号</option>
              <option value="68">EUB重新发货</option>
			...
			<option value="58">提交并打印-平邮</option>
            <option value="59">提交并打印-挂号</option>
        ...
        <th scope='col' nowrap="nowrap"><a href="orderindex.php?module=orders&ostatus=<?php echo $ostatus;?>&action=<?php echo $_REQUEST['action'];?>&sort=ebay_total&sortstatus=<?php echo $sortstatus;?>&account=<?php echo $account;?>&country=<?php echo $country;?>&Shipping=<?php echo $shipping; ?>&start=<?php echo $start;?>&end=<?php echo $end;?>&status=<?php echo $status;?>&isnote=<?php echo $note;?>&ebay_ordertype=<?php echo $ebay_ordertype;?>&keys=<?php echo $keys;?>&searchtype=<?php echo $searchtype;?>"><font color="#0033FF">总价</font></a><?php if($sort == 'ebay_total') echo $sortsimg; ?>&nbsp;</th>
			  ...
			  <?php
				...
				if($ostatus ==0){
					...
					$sql		= "select * from ebay_order as a where ebay_user='$user' and ebay_status='0' ";
				}else{
					$sql		= "select * from ebay_order as a where ebay_user='$user' and ebay_status=$ostatus ";
				}
				if($ostatus == 100){
					$sql		= "select * from ebay_order as a where ebay_user='$user'  ";
				}
...
<td scope='row' align='left' valign="top" ><?php 
							...
							echo date('Y-m-d H:i',$ebay_createdtime);
							}
							 ?>
             ...
             <?php
							...
							for($t=0;$t<count($st);$t++){
								...
								if($imagepic == '' ){
										...
										if($ddsql[0]['goods_pic'] =='' ) $imagepic = 'images/'.$sku.'.jpg';
...
function salereport(){
 ...
 searchorder();
	...
	function orderoperation(type){
		...
		if(typevalue == '58') bybapi('0');
		if(typevalue == '59') bybapi('1');
		
		
		if(typevalue == '63') eubtracknumberv2();
		if(typevalue == '64') eubtracknumberjyv2(0)
		if(typevalue == '65') eubtracknumberrmv2(1)
		if(typevalue == '66') eubtracknumberrmv2(0)
		if(typevalue == '67') eubcanceltracknumberrmv2()
		if(typevalue == '68') eubRetracknumberrmv2()
	...
	//	if(type == 'print'){
			...
			var url		= "byb_applyorder.php?type=creat&bill="+bill+"&type="+type;
		//}
...
function ydapi(type){
	...
	function eubtracknumberv2(){
		...
		var url		= "eubtracknumberv2.php?bill="+bill;
		openwindow(url,'',550,385);
	}
	
		function eubtracknumberjyv2(){
		var bill	= "";
		var checkboxs = document.getElementsByName("ordersn");
		for(var i=0;i<checkboxs.length;i++){
			if(checkboxs[i].checked == true){
		...
		var url		= "eubtracknumberjyv2.php?bill="+bill;
		openwindow(url,'',550,385);
	}
	
	function eubtracknumberrmv2(type){
		var bill	= "";
		var checkboxs = document.getElementsByName("ordersn");
		for(var i=0;i<checkboxs.length;i++){
			if(checkboxs[i].checked == true){
		...
		var url		= "eubtracknumberrmv2.php?bill="+bill+"&pagesize="+type;
		openwindow(url,'',550,385);
	}
	
	
	function eubcanceltracknumberrmv2(type){
		var bill	= "";
		var checkboxs = document.getElementsByName("ordersn");
		for(var i=0;i<checkboxs.length;i++){
			if(checkboxs[i].checked == true){
		...
		var url		= "canceleubtracknumberv2.php?bill="+bill+"&pagesize="+type;
		openwindow(url,'',550,385);
	}
	
	
	function eubRetracknumberrmv2(type){
		var bill	= "";
		var checkboxs = document.getElementsByName("ordersn");
		for(var i=0;i<checkboxs.length;i++){
			if(checkboxs[i].checked == true){
		...
		var url		= "eubtracknumberrev2.php?bill="+bill+"&pagesize="+type;
		openwindow(url,'',550,385);
	}
