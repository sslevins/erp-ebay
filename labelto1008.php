...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', 'OrderNumber');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', 'Name');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', 'E-mail');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', 'Address1');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'Address2');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', 'City');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', 'State');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', 'Zip');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', 'Country');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', 'Province');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', 'Phone');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1', 'Note');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M1', 'SKU');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N1', 'Quantity');
	
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O1', '订单编号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('P1', '订单总金额');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q1', '发货方式');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R1', '货品成本');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('S1', '付款时间');
	...
	for($i=0;$i<count($sql);$i++){
		...
		if($state != ''){
				$addressline .= ",".$state.",".chr(13).$countryname.' '.$zi;
		}
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('O'.$a, $ebay_id, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('P'.$a, $ebay_total, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('Q'.$a, $ebay_carrier, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		for($o=0;$o<count($sl);$o++){
					...
					if(count($rr) > 0){
						...
						for($e=0;$e<count($goods_sncombine);$e++){
							...
							$sq3	= "select goods_cost from ebay_goods where goods_sn='$goods_sn' and ebay_user='$user'";
							$sq3	= $dbcon->execute($sq3);
							$sq3	= $dbcon->getResultArray($sq3);
							$goods_cost += $sq3[0]['goods_cost'];
						}
					}else{
							$sq3	= "select goods_cost from ebay_goods where goods_sn='$sku' and ebay_user='$user'";
							$sq3	= $dbcon->execute($sq3);
							$sq3	= $dbcon->getResultArray($sq3);
							$goods_cost += $sq3[0]['goods_cost']* $ebay_amoun;
					}
		}
		
		
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('R'.$a, $goods_cost, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('S'.$a, $ebay_paidtime, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		for($o=0;$o<count($sl);$o++){
					...
					if(count($rr) > 0){
						...
						for($e=0;$e<count($goods_sncombine);$e++){
							...
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.$a, $strline, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$a, $name, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a, $ebay_usermail, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$a, $street1, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$a, $street2, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$a, $city, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$a, $state, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('H'.$a, $zip, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('I'.$a, $countryname, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('K'.$a, $ebay_phone, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('M'.$a, $goods_sn, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('N'.$a, $goddscount, PHPExcel_Cell_DataType::TYPE_STRING);
							$a++;
							...
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('I'.$a, $countryname, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('K'.$a, $ebay_phone, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('M'.$a, $sku, PHPExcel_Cell_DataType::TYPE_STRING);
							$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('N'.$a, $ebay_amount, PHPExcel_Cell_DataType::TYPE_STRING);
							$a++;
