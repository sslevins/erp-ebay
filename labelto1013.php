...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '库存编码');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '客户备注Custom');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', 'Quantity');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', 'Weight');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'Packing');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', 'Name');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', 'Address Line1');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', 'Address Line2');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', 'Town/City');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', 'State/Province');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', 'Zip/Postal Code');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1', 'Country');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M1', 'Declared Name');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N1', 'Declared Value(USD)');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O1', 'Phone Number');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('P1', 'Email');
	...
	$sql	= "select a.*,b.ebay_amount from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn where ($ertj) and a.ebay_user='$user' and a.ebay_combine!='1' group by a.ebay_id order by   a.ebay_carrier desc ";	
	}
	...
	for($i=0;$i<count($sql);$i++){
	    ...
	    $street2				= str_replace("&acute;","'",$street2);
		...
		$packingname			= '1*1*1';
		}else if($ebay_carrier == '中英专线挂号' ){
		
		$packingname			= '1*1*1';
		}else if($ebay_carrier == '中美专线' ){
		$packingname			= '5*5*5';
		}else if($ebay_carrier == '香港邮政挂号' ){		
		$packingname			= '4*4*4';
		}else if($ebay_carrier == '中英专线' ){
		
		$packingname			= '2*2*2 ';
		}else if($ebay_carrier == '欧陆通' ){
		$packingname			= '2*2*2';
		}
		$ebay_couny				= @$sql[$i]['ebay_couny'];
		$ebay_currency				= @$sql[$i]['ebay_currency'];
		$ss		= "select * from ebay_carrier where ebay_carrier ='$ebay_carrier' and ebay_user ='$user'";
		$ss		= $dbcon->execute($ss);
		...
		for($o=0;$o<count($sl);$o++){					
			...
			$strlinestr				 .= $sku1.' '.$notes.' * '.$amount.' '.$goods_location.' ; ';
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.$a, $skustr, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$a, $appname.$recordnumber0.' , '.$skustr, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a, $qty, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$a, $packingname, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('K'.$a, $zip, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('L'.$a, $countryname, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('M'.$a, $tywsbmc, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('N'.$a, $goods_sbjz, PHPExcel_Cell_DataType::TYPE_STRING);
...
$title		= "CKY".date('Y-m-d');
$titlename		= "CKY".date('Y-m-d').".xls";
