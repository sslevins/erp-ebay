...
if($sort == 'lastsoldtime'  ){
	if($sortstatus  == '0'){
		$sortstr	= " order by a.lastsoldtime desc";
		$sortstatus = 1;
		$sortsimg	= "<img src='images/descend_10x5.gif'   width=\"10\" height=\"5\"/>";
	}else{
		$sortstr	= " order by a.lastsoldtime asc";
		$sortstatus	= 0;
		$sortsimg	= "<img src='images/ascend_10x5.gif'   width=\"10\" height=\"5\"/>";
	}
}
...
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view'>
	<tr class='pagination'>
		...
		<th nowrap="nowrap" scope='col'>Site</th>
                    ...
                    售出</a><?php if($sort == 'QuantitySold') echo $sortsimg; ?></th>
        <th nowrap="nowrap" scope='col'><a href="listing.php?module=list&action=<?php echo $_REQUEST['action'];?>&sort=price&status=<?php echo $_REQUEST['status'];?>&sortstatus=<?php echo $sortstatus;?>&keys=<?php echo $_REQUEST['keys'];?>&searchtype=<?php echo $_REQUEST['searchtype'];?>&ListingType=<?php echo $_REQUEST['ListingType'];?>&account=<?php echo $account;?>">价格</a><?php if($sort == 'price') echo $sortsimg; ?></th>
	                 ...
	                 <th nowrap="nowrap" scope='col'>价格/数量</th>
                     <th nowrap="nowrap" scope='col'>
                     <a href="listing.php?module=list&action=<?php echo $_REQUEST['action'];?>&sort=lastsoldtime&status=<?php echo $_REQUEST['status'];?>&sortstatus=<?php echo $sortstatus;?>&keys=<?php echo $_REQUEST['keys'];?>&searchtype=<?php echo $_REQUEST['searchtype'];?>&ListingType=<?php echo $_REQUEST['ListingType'];?>&account=<?php echo $account;?>" >
                     
                     最后售出时间</a><?php if($sort == 'lastsoldtime') echo $sortsimg; ?>                     </th>
                     <th nowrap="nowrap" scope='col'>售出天数</th>
        <th nowrap="nowrap" scope='col'>操作</th>
			  ...
			  <?php
				...
				for($i=0;$i<count($sql);$i++){
					...
					$lastsoldtime				= $sql[$i]['lastsoldtime'];
					...
					$ffg	= "select a.ebay_paidtime from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn where a.ebay_account ='$ebay_account'  and b.ebay_itemid='$ItemID' and b.sku='$SKU' order by a.ebay_paidtime desc   limit 1 ";
					$ffg		= $dbcon->execute($ffg);
					$ffg		= $dbcon->getResultArray($ffg);
					$ebay_paidtime	= $ffg[0]['ebay_paidtime'];
					if($ebay_paidtime > 0 ){

						$ffgsql	= "update ebay_list set lastsoldtime='$ebay_paidtime' where id =$id ";
						
						
			
						
						$dbcon->execute($ffgsql);
			  ...
			  ?>
				 ...
				 echo $ListingType;
			      ...
			      <td scope='row' align='left' valign="top" ><?php echo $Site;?>&nbsp;</td>
						...
						?>&nbsp;</td>
                        	...
                        	<td scope='row' align='left' valign="top" ><?php if($lastsoldtime>0 ) echo date('Y-m-d H:i:s',$lastsoldtime); ?>&nbsp;</td>
                        	<td scope='row' align='left' valign="top" >
							...
							$days=ceil(($lastsoldtime-strtotime(date('Y-m-d')))/3600/24) ;
							echo abs($days);
                            ...
                            <a href="#" onclick="enditem('<?php  echo $id; ?>')">End Item</a>                            </td>
                        	<?php
							...
							$lastsoldtime		= $ss[$s]['lastsoldtime'];
							$ItemID				= $ss[$s]['itemid'];
							$ebay_account		= $ss[$s]['ebay_account'];
							$SKU				= $ss[$s]['SKU'];
							$iid				= $ss[$s]['id'];
							...
							if($ebay_paidtime > 0 ){
								...
								$ffgsql	= "update ebay_listvariations set lastsoldtime='$ebay_paidtime' where id =$iid ";
								$dbcon->execute($ffgsql);
         		...
         		<tr height='20' class='oddListRowS1'>
         		  ...
         		  <td scope='row' align='left' valign="top" >&nbsp;</td>
         		  <td scope='row' align='left' valign="top" >&nbsp;</td>
         		  <td colspan="4" align='left' valign="top" scope='row' ><?php echo $ss[$s]['VariationSpecifics']; ?>&nbsp;</td>
         		  <td scope='row' align='left' valign="top" >
       		      ...
       		      <td scope='row' align='left' valign="top" ><?php
							...
							$days=round(($lastsoldtime-strtotime(date('Y-m-d')))/3600/24) ;
							echo abs($days);
							}
