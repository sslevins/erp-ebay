<?php
	...
	if($type == 'message'){
		...
		// $ss			= "select * from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where a.ebay_userid='$sendid'";
		 ...
		 if($account){
			$vvv = "select mail from ebay_account where ebay_account='$account'";
			$vvv = $dbcon->execute($vvv);
			$vvv = $dbcon->getResultArray($vvv);
			if($vvv){
				$sale_mail = $vvv[0]['mail'];
			}
		}
		 ...
		 if($ShippedTime != '' && $ShippedTime	!='0'){
			...
			$days = ceil((time()-strtotime($ShippedTime))/86400);
			...
			$content		= str_replace('{Post_Date_today}',$days,$content);
		 ...
		 $content		= str_replace('{seller_email_address}',$sale_mail,$content);
		 ...
		 $content		= str_replace('{ordernumber}',$ordersn,$content);
		...
		$status 	= AddMemberMessageRTQ($mid,$content,$_SESSION['truename'],$mailsent);
		echo $status;
...
echo $xml;
		...
		if($sta == 3){
		$sql			= "UPDATE `ebay_message` SET `status` = '$sta',replyuser='$truename',replytime='$mctime' WHERE `message_id` ='$mid'";
		}else{
		$sql			= "UPDATE `ebay_message` SET `status` = '$sta',`replaycontent` = '$body' WHERE `message_id` ='$mid'";
		}
		
		$sql	= $dbcon->execute($sql);
	
		 $sql	= "select * from ebay_message where message_id='$msid'";
		 $sql	= $dbcon->execute($sql);
		 $sql	= $dbcon->getResultArray($sql);

		 $account		 = $sql[0]['ebay_account'];
		 $sq 			 = "select * from ebay_account where ebay_user='$user' and ebay_account='$account'";
		 $sq		     = $dbcon->execute($sq);
		 $sq		     = $dbcon->getResultArray($sq);
		 $token	         = $sq[0]['ebay_token'];
		 ReviseMyMessages($token,$mid,'Read');
	...
	echo "AAsuccessAA";
		...
		$ordersn	= $_POST['ordersn'];
		$ss			= "select * from ebay_message where id='$ordersn'";
		$ss			= $dbcon->execute($ss);
		$ss			= $dbcon->getResultArray($ss);
		$market		= $ss[0]['mmarket'];
		...
		if($market >=3){
			...
			$ss			= "update ebay_message set mmarket='$market' where id='$ordersn'";
			$ss			= $dbcon->execute($ss);
		}else{
...
$ss					= "insert into ebay_messagenote(note,messageid,addtime,ebay_user,ebay_account,ebay_userid) values('$note','$id','$nowtime','$truename','$ebay_account','$userid') ";
