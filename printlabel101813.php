...
<div style="width:195mm; font-family: ;">
<?php
include "include/config.php";
	$country['Russian Federation']				=	'俄羅斯';
...
$Shipping		= $_REQUEST['Shipping'];
	...
	if($_REQUEST['ordersn'] == ''){
		...
		$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b  on a.ebay_ordersn=b.ebay_ordersn where a.ebay_user='$user' and ebay_combine!='1' ";
	...
	if($ostatus!=='100')
	 ...
	 if($Shipping!='')
			...
			if($hunhe=='2'){
				...
				$sql.=' group by a.ebay_id order by b.sku ';
	}else{
	...
	for($i=0;$i<count($sql);$i++){
		...
		$sn			= $sql[$i]['ebay_ordersn'];
			...
			$countryname1 			= $sql[$i]['ebay_countryname'];
			$countryname 			= $country[$countryname1];
			...
			if(strlen($street1)>35){
				$street = substr($street1,0,35);
				$streets = substr($street1,35);
			}else{
				$street = $street1;
			}
			$citystate = $city.' '.$state.' '.$zip;
			$citystates = '';
			if(strlen($citystate)>30){
				$citystates = substr($citystate,35);
				$citystate = substr($citystate,0,35);
			}
 ?>
	<div style="width:91mm;min-height:62mm;float:left;border:1px dashed black;padding:1mm;font-size:12px; margin:1px;">
		<div style="width:100%;border-bottom:1px dashed #000000;">
			...
			<?php
				$weight		= 0;
				$ee			= "select ebay_amount,sku,ebay_itemtitle,attribute from ebay_orderdetail where ebay_ordersn='$sn'";
				...
				foreach($ee as $key=>$val){
					...
					$t = $key+1;
					if($t%2==0){
						echo '<td width="50%">'.$ebay_amount.'*'.$sku.' '.$attribute.';</td></tr><tr>';
					}else{
						echo '<td width="50%">'.$ebay_amount.'*'.$sku.' '.$attribute.';</td>';
					}
				}
				if($orderweight>0) $weight=$orderweight;
				if($orderweight2>0) $weight=($orderweight2/1000);
			?><br />
Record: <?php echo $recordnumber;?>&nbsp;&nbsp;&nbsp;SHIP: <?php echo $carrier;?>
			</tr>
			</table>
			</div>
			<div style="width:100%;height:3mm; font-weight:bold; line-height:3mm">
				<!--<span style="float:right">SN: <?php echo date('y-m-d').' &nbsp;'.($i+1) ;?>/<?php echo count($sql); ?></span><span style="float:right;margin-right:5px;">NO:<?php echo $ebay_id?></span>-->
                
			</div>
		</div>
		<div style="width:100%;height:4mm;padding-top:1mm; font-size:14px;">
			<div style="width:95%;float:left;">
					<div style='height:4mm;line-height:4mm;width:100%'>From:<strong>lin</strong></div>
			  <div style='font-size:10px;line-height:4mm;margin-left:40px;'>3rd floor,Number 3 building,Eight street,WuHe road,BanTian,LongGang,ShenZhen,518000,China</div>
			</div>
		</div>
		<div style='width:100%; position:relative'><div style='width:95%;float:left; font-size:14px; overflow:hidden;'><ul>
		  <div style='width:100%; position:relative'>
            <div style='width:100%;float:left; font-size:14px; overflow:hidden;'>
              <ul>
                <li style="width:100%;  line-height:5mm">To:<strong><span style='margin-left:20px;font-size:16px'><?php echo $name; ?></span></strong></li>
                <li style="width:100%;  line-height:5mm"><span style='margin-left:38px; font-size:14px; '><?php echo $street;?></span></li>
                <?php if($streets){ ?>
                <li style="width:100%; line-height:5mm"><span style='margin-left:38px;'><?php echo $streets;?></span></li>
                <?php } ?>
                <li style="width:100%;  line-height:5mm"><span style='margin-left:38px;'><?php echo $citystate;?></span></li>
                <?php if($citystates){ ?>
                <li style="width:100%;  line-height:5mm"><span style='margin-left:38px;'><?php echo $citystates;?></span></li>
                <?php } ?>
                <li style="width:100%; line-height:5mm"><span style='margin-left:38px;'><?php echo $countryname1;?></span></li>
                <li style="width:100%; line-height:5mm"><span style='margin-left:38px;'>TEL:<?php echo $tel;?></span></li>
                <li style="width:100%;  line-height:5mm"><span style="float:right;font-size:14px; font-weight:bold;margin-right:25px;"><?php echo $recordnumber;?>&nbsp;SN:<?php echo date('y-m-d').' '.($i+1);?>/<?php echo count($sql); ?></span><span style='margin-left:38px;'>><?php echo $countryname?><</span></li>
              </ul>
            </div>
	  ...
	  <div style='width:100%; margin-top:5px;'>
		<div style='width:100%;'><img src='barcode128.class.php?data=TB<?php echo $ebay_id;?>' width='315' height='35'/>
		</div>
		<div style='width:88%; font-size:14px; margin-top:1px;'>
			<span style='float:left;margin-left:140px'>TB<?php echo $ebay_id;?></span>
		</div>
		</div>
	</div>
