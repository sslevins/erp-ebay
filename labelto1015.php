...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(1)->setCellValue('A1', '地址');
	$objPHPExcel->setActiveSheetIndex(1)->setCellValue('B1', 'sku*数量');
...
$Shipping		= $_REQUEST['Shipping'];
	...
	for($i=0;$i<count($sql);$i++){
		...
		$userid				= substr($ebay_account,0,1);

		$time				= date('m-d',$sql[$i]['ebay_addtime']);
		
		$addressline2 = '';
		
		if($tel == ''){
			$addressline	= $name.chr(10).$street1.chr(10).$city.", ".$state.chr(10).$zip.chr(10).$countryname.'('.$countrynamec.')';
			
		}else{
			$addressline	= $name.chr(10).$street1.chr(10).$city.", ".$state.chr(10).$zip.chr(10).$countryname.'('.$countrynamec.')'.chr(10).$tel;
		}
		
		$addressline	  	= str_replace("&acute;","'",$addressline);
		$addressline  		= str_replace("&quot;","\"",$addressline);
		$ss	= "select sku,ebay_amount from ebay_orderdetail as a where  a.ebay_user='$user' and a.ebay_ordersn = '$ordersn'  ";
		$ss		= $dbcon->execute($ss);
		...
		if($s%2 == 1){
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.$a,$userid.$t, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$a,$time, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'.($a+1).':B'.($a+6));
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.($a+1), $addressline, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(1)->setCellValueExplicit('A'.$c,$userid.$t, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(1)->setCellValueExplicit('B'.$c,$skustr, PHPExcel_Cell_DataType::TYPE_STRING);
			$t++;
			$c++;
			$s++;
		}else{
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a,$userid.$t, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$a,$time, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->mergeCells('C'.($a+1).':D'.($a+6));
