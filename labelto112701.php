...
$country['turkey']					=	'土耳其';
	...
	$country['Zambia']					=	'赞比亚';
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '条码编码');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '买家 ID');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', '图片');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', 'SKU对应的中文名称');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', '数量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '地址');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', 'CODE');
	...
	for($i=0;$i<count($sql);$i++){
		...
		$objPHPExcel->setActiveSheetIndex(0)->getCell('F'.$a)->setValueExplicit($addressline, PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->setActiveSheetIndex(0)->getCell('G'.$a)->setValueExplicit('*'.$ebay_id.'*', PHPExcel_Cell_DataType::TYPE_STRING);
		...
		for($o=0;$o<count($sl);$o++){			
			...
			if($o>= 1) $sku1 .= chr(13).chr(13).chr(13);
			...
			$bb				= "SELECT * FROM ebay_goods where goods_sn='$sku2' ";   
			$bb				= $dbcon->execute($bb);
			...
			if(count($bb)==0){
				...
				$rr			= "select * from ebay_productscombine where ebay_user='$user' and goods_sn='$sku2'";   
				$rr			= $dbcon->execute($rr);
			...
			$objPHPExcel->setActiveSheetIndex(0)->getCell('D'.$tt)->setValueExplicit($sku1.' '.$goods_name, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('E'.$tt)->setValueExplicit($amount, PHPExcel_Cell_DataType::TYPE_STRING);
			...
			$objDrawing->setCoordinates('C'.$tt);
...
$objPHPExcel->getActiveSheet(0)->getStyle('G2:G500')->getFont()->setName('C39HrP24DhTt');
