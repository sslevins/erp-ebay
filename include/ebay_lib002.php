...
<GetSellingManagerSaleRecordRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <TransactionID>0</TransactionID>
		...
		function stockused($goods_sn,$storeid){
			global $dbcon,$user;
			/* 检查此sku是否是组合产
			$vv				= "select * from ebay_goods where goods_sn ='$goods_sn' and ebay_user ='$user' ";
			$vv				= $dbcon->execute($vv);
			$vv				= $dbcon->getResultArray($vv);品 */
			$usedqty		= 0;
		...
		foreach((array)$Trans as $Transaction){
			...
			if($orderstatus == '1' && $Name != '' ){
				...
				if(count($vv) == 0 ){
					...
					$sql			.= " ,`ebay_currency` ,`ebay_total` ,`ebay_status`,`ebay_user`,`ebay_shipfee`,`ebay_account`,`recordnumber`,`ebay_addtime`,`ebay_note`,`ebay_site`,`eBayPaymentStatus`,`PayPalEmailAddress`,`ShippedTime`,`RefundAmount`,`ebay_warehouse`)VALUES ('$CompleteStatus','$OrderID', '$tid' , '$ExternalTransactionID' , '$OrderID' , '$CreatedTime' , '$PaidTime' , '$BuyerUserID' ,";
					$sql			.= " '$Name' , '$Emails' , '$Street1' , '$Street2' , '$CityName','$StateOrProvince' , '$Country' , '$CountryName' , '$PostalCode' , '$Phone' , '$Currency' , '$AmountPaid' , '$orderstatus','$user','$ShippingServiceCost','$account','$addrecordnumber','$mctime','$BuyerCheckoutMessage','$site','$eBayPaymentStatus','$PayPalEmailAddress','$ShippedTime','$RefundAmount','$defaultstoreid')";
					if($dbcon->execute($sql)){
					if($Email != ''){
						$detailline		=  array();
		...
		if($ReturnedTransactionCountActual == 1){
			...
			foreach((array)$Trans as $Transaction){
				...
				if($status == "0" && $Name != ''){
					...
					$sql			.= "`ebay_createdtime` ,`ebay_paidtime` ,`ebay_userid` ,`ebay_username` ,`ebay_usermail` ,`ebay_street` ,";
					$sql			.= "`ebay_street1` ,`ebay_city` ,`ebay_state` ,`ebay_couny` ,`ebay_countryname` ,`ebay_postcode` ,`ebay_phone`";
					$sql			.= " ,`ebay_currency` ,`ebay_total` ,`ebay_status`,`ebay_user`,`ebay_shipfee`,`ebay_account`,`recordnumber`,`ebay_addtime`,`ebay_note`,`ebay_site`,`eBayPaymentStatus`,`PayPalEmailAddress`,`ShippedTime`,`RefundAmount`,`ebay_warehouse`)VALUES ('$CompleteStatus','$val', '$tid' , '$ptid' , '$orderid' , '$CreatedDate' , '$PaidTime' , '$UserID' ,";
					$sql			.= " '$Name' , '$Email' , '$Street1' , '$Street2' , '$CityName','$StateOrProvince' , '$Country' , '$CountryName' , '$PostalCode' , '$Phone' , '$Currency' , '$AmountPaid' , '$orderstatus','$user','$shipingfee','$account','$addrecordnumber','$mctime','$BuyerCheckoutMessage','$site','$eBayPaymentStatus','$PayPalEmailAddress','$ShippedTime','$RefundAmount','$defaultstoreid')";
		...
		while(true){    		
			...
			if(array_key_exists('L_TRANSACTIONID'.$sure,$ebayArray)){
					...
					//		$tid			= '13427964SF159073P';
							...
							$vv			= "select * from ebay_order where ebay_ptid ='$tid' and ebay_user ='$user' ";
							$vv			= $dbcon->execute($vv);
							$vv			= $dbcon->getResultArray($vv);
...
*/
	...
	function getordersn($ebayaccount){
		...
		$ss			= "select * from ebay_rand where time='$nowd' and ebay_account ='$ebayaccount' order by id desc ";
		$ss			= $dbcon->execute($ss);
		$ss			= $dbcon->getResultArray($ss);
		if(count($ss) == 0){
			$value		= 1;
		...
		$sql		= "insert into ebay_rand(time,value,ebay_account) values('$nowd','$value','$ebayaccount')";
		$dbcon->execute($sql);
		return $value;
	}
