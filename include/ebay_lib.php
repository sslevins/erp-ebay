<?php	
		...
		function GetPurchasePrice($goods_sn){
			...
			/* 取得最新的采购价格 */
			$vvsql		= "select goods_cost from ebay_iostore as a join ebay_iostoredetail as b on a.io_ordersn	= b.io_ordersn	 where (a.type ='0' or a.type='2' or a.type ='99' or a.type ='98'  or a.type ='97' or a.type ='96' or a.type ='93' or a.type ='94') and b.goods_sn ='$goods_sn' order by    a.id desc  ";
			...
			/* 计算平均价 */
			$vvsql		= "select sum(goods_cost*goods_count) as total,sum(goods_count) as totalcount from ebay_iostore as a join ebay_iostoredetail as b on a.io_ordersn	= b.io_ordersn	 where (a.type ='0' or a.type='2' or a.type ='99' or a.type ='98'  or a.type ='97' or a.type ='96' or a.type ='93' or a.type ='94') and b.goods_sn ='$goods_sn' order by    a.id desc  ";
			$vvsql	 = $dbcon->execute($vvsql);
			$vvsql	 = $dbcon->getResultArray($vvsql);
		...
		*/
		...
		function getProductsqty($start,$end,$goods_sn,$io_warehouse){
			...
			*/
			$gsql			= "SELECT sum(b.ebay_amount) as qty FROM ebay_order AS a JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn WHERE sku =  '$goods_sn' and a.ebay_warehouse = '$io_warehouse' and a.ebay_status!='8888'";
				...
				for($i=0;$i<count($vv);$i++){
					...
					for($j=0; $j<count($fxgoods_sncombine);$j++){
						...
						if(strstr($fxlaberstr,$goods_sn)){							
							...
							$gsql			= "SELECT sum(b.ebay_amount) as qty FROM ebay_order AS a JOIN ebay_orderdetail AS b ON a.ebay_ordersn = b.ebay_ordersn WHERE sku =  '$cgoods_sn'   and a.ebay_warehouse = '$io_warehouse' and a.ebay_status!='8888'";
							if($start != '' && $end != '') $gsql 			.= " and (a.ebay_paidtime	>=".strtotime($start)." and a.ebay_paidtime	<=".strtotime($end).")";
...
die();
		...
		<RuName>Survy-Survyc487-9ec7--nbhygx</RuName>
		</GetSessionIDRequest>
		';			
		$session = new eBaySession($token, $devID, $appID, $certID, $serverUrl, $compatabilityLevel, $siteID, $verb);   
		$responseXml = $session->sendHttpRequest($requestXmlBody);
	...
	function getItem($token,$itemid,$picvalue){
		...
		if($picvalue != '' && $Variations != '' && $data['GetItemResponse']['Item']['Variations']['Pictures'] != '' ){
		...
		foreach((array)$Trans as $Transaction){
			...
			if($orderstatus == '1' && $Name != '' ){
				...
				if(count($vv) == 0 ){
					...
					foreach((array)$detailline as $dline){
							...
							if(count($bb) == 0 ){
									...
									if(strstr($SKU,'#')){
										...
										echo '此sku 转换后结果是'.$BuyerUserID.' '.$SKU;
										
									}else{
										echo '此sku 转no后结果是'.$BuyerUserID.$SKU;
									...
									if($user == 'vipzz'){
										...
										$ggsql		= "select * from ebay_goods where goods_sn='$SKU' and ebay_user='$user' ";	
										
										
										echo $ggsql;
										
										$ggsql				= $dbcon->execute($ggsql);
										$ggsql				= $dbcon->getResultArray($ggsql);
										...
										if(count($ggsql) == 0){
											...
											$goods_pic	= $SKU.".jpg";
											file_put_contents($filepath."\\images\\".$goods_pic,$img);
											$ggsql = "insert into ebay_goods(goods_sn,goods_pic) values('$SKU','$goods_pic')";
											$dbcon->execute($ggsql);	
											
										}else{
											
											
											$goods_id		= $ggsql[0]['goods_id'];
											$goods_pic		= $ggsql[0]['goods_pic'];
											if($goods_pic == '' ){
												...
												//echo $filepath."\\images\\".$goods_pic;
												$ggsql = "update ebay_goods set goods_pic='$goods_pic' where goods_id='$goods_id' and ebay_user='$user'  ";	
					...
					if(strlen($ExternalTransactionID) >= 8 && ($user =='viphcx' ||  $user =='survy' ||  $user =='feifei' ||  $user =='vidpddd' ||  $user =='vip929' ||  $user =='vip778'  ) ){
	...
	function GetSellerTransactions02($start,$end,$token,$account,$type,$id){
		...
		<GetSellerTransactionsRequest xmlns="urn:ebay:apis:eBLBaseComponents">
		  <RequesterCredentials>
			<eBayAuthToken>'.$token.'</eBayAuthToken>
		     ...
		     <DetailLevel>ReturnAll</DetailLevel>
			 <OutputSelector>TransactionArray.Transaction.Variation</OutputSelector>
    	...
    	$responseXml = $session->sendHttpRequest($requestXmlBody);	
			...
			if($user =='viperp') 	file_put_contents('s.txt', $responseXml,FILE_APPEND);
...
print_r($data);
		...
		if($ReturnedTransactionCountActual == 1){
			...
			foreach((array)$Trans as $Transaction){
				...
				if($Transaction['Variation'] != '' ){
							...
							$aname			    = $Transaction['Variation']['VariationSpecifics']['NameValueList']['Name'];
							$avalue			    = $Transaction['Variation']['VariationSpecifics']['NameValueList']['Value'];
							$aavalue		    = $Transaction['Variation']['VariationSpecifics']['NameValueList']['Value'];
							...
							if($aname == ''){
							for($e=0;$e<count($Variation);$e++){
							$aname			= $Transaction['Variation']['VariationSpecifics']['NameValueList'][$e]['Name'];
							$avalue			= $Transaction['Variation']['VariationSpecifics']['NameValueList'][$e]['Value'];
							$arrribute	.= $aname.":".$avalue;
				...
				if($status == "0" && $Name != ''){
					...
					if(count($ss) == 0){
							if($dbcon->execute($sql)){
								...
								AddorderDetail($val,$ItemID,$Title,$CurrentPrice,$QuantityPurchased,$CreatedDate,$shipingservice,$sku,$shipingfee,$account,$mctime,$pic,$site,$token,$recordnumber,$CountryName,$ListingType,$tid,$FeeOrCreditAmount,$FinalValueFee,$arrribute,$BuyerCheckoutMessage,$picvalue);
								
								
								if(strlen($ptid) >= 8 && ($user =='viphcx' ||  $user =='survy' ||  $user =='luoxiaoming'  ||  $user =='vipshen' ) ){
					...
					$sql	    = $dbcon->getResultArray($sql);
					if(count($sql) == 0){
					AddorderDetail($status,$ItemID,$Title,$CurrentPrice,$QuantityPurchased,$CreatedDate,$shipingservice,$sku,$shipingfee,$account,$mctime,$pic,$site,$token,$recordnumber,$CountryName,$ListingType,$tid,$FeeOrCreditAmount,$FinalValueFee,$arrribute,$BuyerCheckoutMessage,$picvalue);
	...
	function AddorderDetail($sn,$iid,$title,$price,$amount,$ctime,$shiptype,$sku,$shipingfee,$account,$mctime,$pic,$site,$token,$recordnumber,$CountryName,$ListingType,$tid,$FeeOrCreditAmount,$FinalValueFee,$arrribute,$BuyerCheckoutMessage,$picvalue){
		global $user,$dbcon;
			...
			if($FolderID != 1){
				...
				foreach((array)$Trans as $Transaction){
					...
					if($dbcon->execute($sql)){
						...
						$mid		= mysql_insert_id();
						$msql		= "insert into ebay_messagedetail(body,reply,messageid) values('$Body','','$mid')";
						$dbcon->execute($msql);
			...
			if(array_key_exists('L_TRANSACTIONID'.$sure,$ebayArray)){
					...
					//		$tid			= '13427964SF159073P';
							...
							if(count($vv) == '0' && $TRANSACTIONTYPE == 'sendmoney'  ){
								...
								$sql			.= " '$Name' , '$Emails' , '$Street1' , '$Street2' , '$CityName','$StateOrProvince' , '$Country' , '$CountryName' , '$PostalCode' , '$Phone' , '$L_CURRENCYCODE' , '$gross' , '$paypalstatus','$user','$ShippingServiceCost','$account','$addrecordnumber','$mctime','$NOTE','$site','$eBayPaymentStatus','$PayPalEmailAddress','$defaultstoreid')";
								$dbcon->execute($sql);
