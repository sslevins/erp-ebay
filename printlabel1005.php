ppue<?php
	...
	$country['Zimbabwe']				=	'津巴布韦';
		...
		$ertj		= "";
			...
			$sql	= "select a.*,b.sku from ebay_order where ($ertj) and ebay_user='$user'";
			
			
			
			$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where  ($ertj) group by a.ebay_id order by b.sku  asc ";
...
<style type="text/css">
<!--
.STYLE1 {font-family: C39HrP24DhTt;
font-size: 55px;
...
<?php
 	...
 	for($i=0;$i<count($sql);$i++){
				...
				if($street2 == ''){
					...
					$addressline	= "Send To:".$cname."<br>".$street1."<br>".$city.", ".$state."<br>".$zip."<br>".$countryname;
				
				}else{
				
					$addressline	= "Send To:".$cname."<br>".$street1."<br>".$street2."<br>".$city.", ".$state."<br>".$zip."<br>".$countryname;
					
				}
				
				
				$addressline .= '('.$country[$countryname].')';
				
				
				$sl	= "select * from ebay_account where ebay_account='$ebayaccount' and ebay_user='$user' ";
				$sl	= $dbcon->execute($sl);
				...
				$sl	= "select * from ebay_carrier where name='$ebay_carrier' and ebay_user='$user' ";
				$sl	= $dbcon->execute($sl);
				$sl	= $dbcon->getResultArray($sl);
				$slvalue	= $sl[0]['value'];
				$address0	= "From:".nl2br($sl[0]['address']);
				
				$sldh		= $ebayaccount."_".$sl[0]['carrier_sn'];
...
<table height="214" border="0" cellpadding="0" cellspacing="0" style=" width:95mm; height:60mm" >
  ...
  <tr>   
    ...
    <table width="100%" height="100" border="0" cellpadding="0" cellspacing="0" style="border:1px dashed #999999">
      ...
      <tr>
        ...
        <td><div align="right">
          <table width="100%" border="0" cellspacing="0" cellpadding="1" style="border-bottom:5px #000000 solid">
            <tr>
              <td>
			  <?php
              ...
              <td rowspan="2" valign="top"><div align="right">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
                    <td align="right">
                      <div align="right"><img src="barcode128.class.php?data=<?php echo $ebay_id; ?>" alt="" width="137" height="40"/></div><?php echo $ebay_id;?></td>
                  </tr>
                </table>
                </div></td>
            </tr>
            <tr>
              <td><?php echo $slvalue;?>&nbsp;</td>
              </tr>
          </table>
        </div></td>
      </tr>
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-bottom:5px #000000 solid">
          <tr>
            <td width="50%" rowspan="2" valign="top"><span class="STYLE4"><?php echo $address0; ?></span>              <div align="center"><br />
            </div></td>
            <td width="50%" valign="top"><span class="STYLE4"><?php echo $addressline; ?> &nbsp;</span></td>
          </tr>
          <tr>
            <td valign="top">Phone:<?php echo $tel;?></td>
          </tr>
      ...
      <tr>
        ...
        <td height="35"><table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" bgcolor="#000000">
        <?php
					...
					if(count($rr) > 0){
									...
									for($e=0;$e<count($goods_sncombine);$e++){
											...
											$goddscount     = $pline[1] * $amount;
											 ...
											 $ee			= "SELECT a.goods_name,a.goods_location,sum(b.goods_count) as goods_count FROM ebay_goods as a join ebay_onhandle as b on a.goods_id = b.goods_id where a.goods_sn='$goods_sn' and b.store_id='$storeid' and a.ebay_user='$user' group by a.goods_sn";
											...
											$ee 	 	= $dbcon->getResultArray($ee);
													  ...
													  if(count($ee) == 0 ){
			  										  $ee			= "SELECT a.goods_name FROM ebay_goods as a where a.goods_sn='$goods_sn' and a.ebay_user='$user' ";
					 									$ee			= $dbcon->execute($ee);
														$ee 	 	= $dbcon->getResultArray($ee);
		...
		?>
          ...
          <tr>
            <td bgcolor="#FFFFFF"><font size="2px"><?php echo $goods_sn;?></font>&nbsp;</td>
            <td bgcolor="#FFFFFF"><font size="2px"><?php echo $goods_name;?></font>&nbsp;</td>
            <td bgcolor="#FFFFFF" align="center"><font size="2px"><?php echo $goddscount;

			if($goddscount > $goods_count2) echo '<0';
		  ...
		  $ee			= "SELECT a.goods_name,a.goods_location,sum(b.goods_count) as goods_count FROM ebay_goods as a join ebay_onhandle as b on a.goods_id = b.goods_id where a.goods_sn='$sku' and a.ebay_user='$user' and b.store_id='$storeid' group by a.goods_sn";

		 $ee			= $dbcon->execute($ee);
											$ee 	 	= $dbcon->getResultArray($ee);
					  if(count($ee) == 0 ){
			  	  $ee			= "SELECT a.goods_name FROM ebay_goods as a where a.goods_sn='$sku' and a.ebay_user='$user' ";
					 $ee			= $dbcon->execute($ee);
											$ee 	 	= $dbcon->getResultArray($ee);
		   ...
		   ?>
              ...
              <tr>
            <td bgcolor="#FFFFFF"><font size="2px"><?php echo $sku;?></font>&nbsp;</td>
            <td bgcolor="#FFFFFF"><font size="2px"><?php echo $goods_name;?></font>&nbsp;</td>
            <td bgcolor="#FFFFFF" align="center"><font size="2px"><?php echo $amount;

			if($amount > $goods_count2) echo '<0';
          ...
          <div align="right">
			...
			if($ebay_noteb != '') echo $ebay_noteb.'<br>';
