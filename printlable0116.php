...
$tj = '';
	...
	if($_REQUEST['ordersn'] == ''){
		$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where a.ebay_status='$ostatus' and a.ebay_user='$user' and ebay_combine!='1' group by a.ebay_id order b.sku";
	}else{
		
		
		$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where ($tj) group by a.ebay_id order by a.ebay_id asc ";
	...
	for($i=0;$i<count($sql);$i++){ 
		...
		if($user =='vipvee' && strlen($countryname) == 2 ){
		
			$vv			= "select ebay_countryname  from ebay_order where ebay_couny ='$countryname' order by ebay_countryname 	 desc ";
			$vv			= $dbcon->execute($vv);
	 		$vv			= $dbcon->getResultArray($vv);
			$countryname		= $vv[0]['ebay_countryname'];
		}
		...
		$addline = 'TO:'.$name.'<br>'.$street.'<br>'.$citystate.'<br>'.$countryname.'('.$countf_name.')';
		if($tel)$addline .='<br>TEL:'.$tel;
		
		
		if($user == 'vipvee' ) $ebay_id = 'De'.$ebay_id;
		
		
		
		
 ?>
	<td style="width:68mm;height:40mm;border:1px solid #000000;font-size:12px;">
		<table  width="100%" height="100" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td style='width:9mm;height:6.4mm;border-right:1px solid #000000;font-size:14px;line-height:6.4mm;text-align:center;'>
				<strong><?php echo $i+1;?></strong>
			</td>
			<td style='width:37mm;border-right:1px solid #000000;text-align:center;'><?php $listsn = substr($sn,0,19);echo $listsn;?></td>
			<td style='width:22mm; text-align:center;'>
				<?php echo $ebay_id;?>
			</td>
		</tr>
		<tr>
			<td colspan="2" style='border-right:1px solid #000000;border-top:1px solid #000000;'><div style='float:left;margin-left:3px;min-height:24mm;line-height:4mm;'><?php echo $addline;?></div></td>
			<td style='border-top:1px solid #000000;'><img src="barcode128.class.php?data=<?php echo $ebay_id;?>" alt="" width="48" height="24" style='float:right;width:20mm;height:8mm;margin:5mm 1mm;'/></td>
		</tr>
		<tr>
			<td colspan="3" style='border-top:1px solid #000000; height:5mm; line-height:5mm; padding-left:3px;'>
			<?php 
		...
		<tr>
			<td colspan="2" style='border-top:1px solid #000000; border-right:1px solid #000000; height:5mm;line-height:5mm; padding-left:3px;'><?php echo $carrier;?>
			</td>
			<td style='border-top:1px solid #000000; line-height:5mm;'><?php echo $ebay_tracknumber;?>&nbsp;</td>
		</tr>
		</table>
	</td>
	...
	if($rrr%3==0){
		...
		$hang++;
		if($hang==7 ){
			echo '</table><div style="page-break-after:always;"></div><table style="width:100%; cellpadding="0" cellspacing="0"><tr>';
			$hang = 0;
			$rrr = 0;
		}else{
			echo "<tr>";
