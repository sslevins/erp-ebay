...
<?PHP
	...
	function eubtracknumber($ordersn){
		...
		if($ebay_usermail == 'Invalid Request') $ebay_usermail = 'cc@126.com';
		...
		*/
		...
		$ss			= "select * from  ebay_orderdetail where ebay_ordersn = '$ebay_ordersn' and recordnumber != '' ";
		$ss			= $dbcon->execute($ss);
		$ss			= $dbcon->getResultArray($ss);
		...
		if($user == 'chineon') $sku .= $si[0]['goods_note'];
			...
			if(count($rr) > 0){
				...
				for($z=0;$z<count($goods_sncombine);$z++){
					...
					$weights		= $si[0]['goods_weight']?$si[0]['goods_weight']:0.1;
		...
		$ebay_itemtitle		= str_replace('&quot;','',$ebay_itemtitle);
		$weight				= 0.01;
		$goods_ywsbmc		= $ebay_itemtitle;
		$goods_zysbmc		= $ebay_itemtitle;
		$goods_sbjz			= 1;
		...
		$item[$i]		= array(
		'CurrencyCode' => trim($ebay_currency),
		...
		'SKUID' => $sku,
		'Weight' => $weight * $ebay_amount,
		'CustomsTitleCN' => $goods_zysbmc.' ['.$sku.' * '.$ebay_amount.']',
		'CustomsTitleEN' => $goods_ywsbmc,
		...
		/* get hk post zone 
		$soapclient = new soapclient("http://shippingapi.ebay.com.hk/production/v1/orderhkservice.asmx?WSDL");
		...
		$functions = $soapclient->GetHKPostZone(array("GetHKPostZoneRequest"=>$params));
		print_r($functions);
    	...
    	'APIPassword' => $APIPassword,
    	'APISellerUserID' => $APISellerUserID,
    	'MessageID' => "135625622432",'TrackCode' => "LK012439797HK",
	);
		
		
	
		$functions = $soapclient->GetHKShippingPackage(array("GetHKShippingPackageRequest"=>$params));
		print_r($functions);
	...
	*/
    	...
    	'APISellerUserID' => $APISellerUserID,
    	'MessageID' => "135625622432",
		"OrderDetail"=> array("PickUpAddress"=>$PickUpAddress,"ShipFromAddress"=>$ShipFromAddress,"ShipToAddress"=>$ShipToAddress,"ItemList"=>array("Item"=>$item),"ShipCode"=>0,"Zone"=>"USB","ItemCategory"=>'G',"InsuranceAmount"=>0,"InsuranceTypeCode"=>0,"MailType"=>2,"ECustoms"=>false));
		
		
	print_r($params);
	
		$functions = $soapclient->AddHKShippingPackage(array("AddHKShippingPackageRequest"=>$params));
