...
date_default_timezone_set ("Asia/Chongqing");	
	...
	function getmerge($ordersn){
		...
		foreach($skuary as $key=>$value){
			...
			$totalsku			.= $sku."*".($amount*$goods_register)." ".$goods_name."<br>";
	...
	$sql1	= "select sku,sum(b.ebay_amount) from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn where a.ebay_user='$user' and a.ebay_status='1' and a.ebay_combine!='1' ";
	
	}else{	
	$sql	= "select * from ebay_order as a where ($ertj) and ebay_user='$user' and a.ebay_combine!='1' ";	
	$sql1	= "select sku,sum(b.ebay_amount) from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn where a.ebay_user='$user' and ($ertj) and a.ebay_combine!='1' ";
	...
	if($iscombine == 1){
		...
		$sql	.= " GROUP BY ebay_userid, ebay_username, ebay_street, ebay_street1 ORDER BY ebay_userid";
		
	
	}
	
	$sql1   .= "  GROUP BY b.sku ORDER BY b.sku";

	//$sql	= "SELECT * FROM ebay_order AS a WHERE ebay_user = 'vipliang' AND a.ebay_status = '1' AND a.ebay_combine != '1' GROUP BY ebay_userid, ebay_username, ebay_street, ebay_street1 ORDER BY ebay_userid";
	...
	for($i=0;$i<count($sql);$i++){
		...
		if($iscombine == 1){
		$ss							= "select * from ebay_order as a where ebay_status='$ebay_status' and ebay_userid='$ebay_userid' and ebay_username='$name' and ebay_street='$street1' and ebay_street1='$street2' and ebay_combine!='1' and ebay_ordersn!='$ordersn' and ($ertj)";
		$ss	= $dbcon->execute($ss);
		$ss	= $dbcon->getResultArray($ss);
		for($u=0;$u<count($ss);$u++){
			...
			$sl				= "select * from ebay_orderdetail where ebay_ordersn='$uorder'";
			$sl				= $dbcon->execute($sl);
			$sl				= $dbcon->getResultArray($sl);
			...
			for($o=0;$o<count($sl);$o++){			
				...
				$totalsku		.= $sku."*".$amount." ".$goods_name."<br>";
		...
		$rightstr					 = ($i+1).". ".$ebay_account.":".$recordnumber."<br>".$totalsku;
		
		$output						.='<span class="STYLE1"><table width="100%" height="180" border="1" align="center" cellpadding="0" cellspacing="0">
...
<tr>
  <td height="15" colspan="2" align="center" valign="top"><div align="left">'.$ebay_note."  ".$ss.'</div></td>
  </tr>
</table></span>
';
	...
	for($i=0;$i<count($sql1);$i++){
			...
			$linestr	= $sku."*".($qty*$goods_register)."<br>";
			$output		.= $linestr;
