	...
	for($j=0;$j<count($vv);$j++){
		...
		for($i=0;$i<count($sql);$i++){
			...
			$account	= $sql[$i]['ebay_account'];
				...
				$start		= date('Y-m-d H:i:s',strtotime("$cc -520 minutes"));
	...
	function GetOrderSiptype($user){
		...
		global $dbcon;
			$sql		= "select * from ebay_order as a where ebay_user='$user'  and ebay_combine!='1' and  ebay_status = '1' and (ebay_carrier = '' or ebay_carrier is null) ";
					//	$sql		= "select * from ebay_order as a where ebay_user='$user'  and ebay_id = '11692'  ";
					...
					//	echo $sql;
						...
						for($i=0;$i<count($sql);$i++){
							...
							$totalweight			= 0;
							$goods_cost				= 0;
							$st	= "select sku,ebay_amount,ebay_shiptype from ebay_orderdetail where ebay_ordersn='$ebay_ordersn'";
							$st = $dbcon->execute($st);
							...
							$ebay_shiptype	= $st[0]['ebay_shiptype'];
