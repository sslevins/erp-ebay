...
while(true){
	...
	if($recordnumber != ''){
	$ebay_userid				= str_rep(trim($currentSheet->getCell($dd)->getValue())); //买家名称
	$ebay_username				= str_rep(trim($currentSheet->getCell($oo)->getValue())); //买家名称
	$ebay_usermail				= str_rep(trim($currentSheet->getCell($ee)->getValue())); // 买家邮箱
	$ebay_createdtime			= trim($currentSheet->getCell($ff)->getValue()); // 下单时间
	$ebay_paidtime				= trim($currentSheet->getCell($gg)->getValue()); // 付款时间
	$ebay_paidtime					= strtotime(str_replace('.','-',$ebay_paidtime));
	$ebay_createdtime				= strtotime(str_replace('.','-',$ebay_createdtime));
	
	
	$ebay_itemprice				= str_rep(trim($currentSheet->getCell($hh)->getValue()));   //产品总金额
	$ebay_shipfee				= str_rep(trim($currentSheet->getCell($ii)->getValue()));   // 物流费用
	$ebay_total					= str_rep(trim($currentSheet->getCell($jj)->getValue()));   //订单金额
	$productsinformation					= str_rep(trim($currentSheet->getCell('L'.$c)->getValue()));   
	...
	for($j=0; $j<count($productsinformation);$j++){
			...
			$labelstr		= str_replace('性:','性: ',$labelstr);
			$labelstr		= str_replace('码:','码: ',$labelstr);
			$labelstr		= str_replace('量:','量: ',$labelstr);
			
			if($labelstr != '' ){
			
			
				$data 		= explode(': ',$labelstr);
				$title		= substr($data[0],5);
				$title		= str_replace('(商家编码','',$title);
				$title		= str_replace('(产品规格','',$title);
				$title		= str_replace(')','',$title);
				
	
				
				
				if(strstr($title,"产品属性")){
				
				$title		= str_replace('(产品属性','',$title);
				...
				$skulocation= strpos($sku,')');
				$sku		= substr($sku,0,$skulocation);
				
					echo $sku;
				
				$qty		= $data[3];
				$qtylocation= strrpos($qty,' ');
				$qty		= substr($qty,0,$qtylocation);
				...
				$dataarray[$jj]['sku']	= trim($sku);
				$dataarray[$jj]['qty']	= trim($qty);
				$dataarray[$jj]['title']	= trim($title);
				$jj++;
	...
	$ebay_note					= str_rep(trim($currentSheet->getCell($mm)->getValue()));	//订单备注
	$ebay_countryname			= str_rep(trim($currentSheet->getCell($pp)->getValue()));
	$ebay_state					= str_rep(trim($currentSheet->getCell($qq)->getValue()));
	$ebay_city					= str_rep(trim($currentSheet->getCell($rr)->getValue()));
	$ebay_street				= str_rep(trim($currentSheet->getCell($ss)->getValue()));
	$ebay_postcode				= str_rep(trim($currentSheet->getCell($tt)->getValue()));
	$ebay_phone					= str_rep(trim($currentSheet->getCell($uu)->getValue()));
	$ebay_carrier				= str_rep(trim($currentSheet->getCell($ww)->getValue()));   //买家选择物流
	$develiverytime				= str_rep(trim($currentSheet->getCell($xx)->getValue()));   //发货期限
	$ebay_phone1				= str_rep(trim($currentSheet->getCell($vv)->getValue()));   //发货期限
	
	
	if($ebay_carrier   == 'Hongkong Post Air Mail'){
	$ebay_carrier		= '香港小包挂号';
	}
	
	if($ebay_carrier   == 'UPS Express Saver'){
	$ebay_carrier		= 'UPS';
	}
	...
	if($ebay_carrier   == 'ChinaPost Post Air Mail'){
	$ebay_carrier		= '中国邮政挂号';
	}
	
	if($ebay_carrier   == 'China Post Air Mail'){
	$ebay_carrier		= '中国邮政挂号';
	}
	...
	if($vv == '0'){
		...
		$sql					= "insert into  ebay_order(ebay_ordersn,ebay_paystatus,recordnumber,ebay_tid,ebay_ptid,ebay_orderid,ebay_createdtime,ebay_paidtime,ebay_userid,ebay_username,ebay_usermail,ebay_street,ebay_street1,ebay_city,ebay_state,ebay_countryname,ebay_postcode,ebay_phone,ebay_total,ebay_status,ebay_user,ebay_addtime,ebay_shipfee,ebay_account,ebay_note,ebay_carrier,ebay_warehouse,ebay_currency,ebay_phone1) values('$val','Complete','$recordnumber','$ebay_tid','$ebay_ptid','$ebay_orderid','$ebay_createdtime','$ebay_paidtime','$ebay_userid','$ebay_username','$ebay_usermail','$ebay_street','$ebay_street1','$ebay_city','$ebay_state','$ebay_countryname','$ebay_postcode','$ebay_phone','$ebay_total','$orderstatus','$user','$mctime','$ebay_shipfee','$account','$ebay_note','$ebay_carrier','$defaultstoreid','USD','$ebay_phone1')";
		...
		if($dbcon->execute($sql)){
						...
						$sql = "select * from ebay_orderdetail where ebay_ordersn='$vv' and recordnumber='$recordnumber'";
		$sql = $dbcon->query($sql);
		...
		for($j=0;$j<count($dataarray);$j++){
			...
			if($j>=1) $recordnumber		= $recordnumber.$j;
		...
		for($j=0;$j<count($dataarray);$j++){
			...
			if($j> 0) $recordnumber		= $recordnumber.$j;
				...
				$esql	 = "INSERT INTO `ebay_orderdetail` (`ebay_ordersn` ,`ebay_itemid` ,`ebay_itemtitle` ,`ebay_itemprice` ,";
				$esql    .= "`ebay_amount` ,`ebay_createdtime` ,`ebay_shiptype` ,`ebay_user`,`sku`,`shipingfee`,`ebay_account`,`addtime`,`ebay_itemurl`,`ebay_site`,`recordnumber`,`storeid`,`ListingType`,`ebay_tid`,`FeeOrCreditAmount`,`FinalValueFee`,`attribute`,`notes`,`goods_location`)VALUES ('$vv', '$iid' , '$title' , '$ebay_itemprice' , '$amount'";
	...
	if($recordnumber == '') break;
						...
						$sql		= "select * from ebay_order as a where ebay_user='$user'  and ebay_combine!='1' and ebay_status = '595'  ";
						$sql	= $dbcon->execute($sql);
						$sql	= $dbcon->getResultArray($sql);
