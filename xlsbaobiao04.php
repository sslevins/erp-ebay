...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1','扫描出库记录');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1','所有售出记录');
...
where a.ebay_user='$user'  and a.ebay_combine!='1' and (a.scantime >=$start and a.scantime <=$end)   GROUP BY b.sku ";	
	...
	for($i=0;$i<count($sql);$i++){
		...
		$sku				= $sql[$i]['sku'];	
		$totalcount				= $sql[$i]['totalcount'];
		
		/* 计算某个 sku 所有的售出记录  2012-05-11 13:00 */
			$vv		= "select sum(ebay_amount) as total from ebay_orderdetail where sku='$sku' ";
			$vv		= $dbcon->execute($vv);
			$vv		= $dbcon->getResultArray($vv);
			$vvtotal= $vv[0]['total'];
		/*  */	
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$a, $sku);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$a, $totalcount);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$a, $vvtotal);
