...
if($type == 'creat'){
	...
	foreach($sql as $key=>$val){
		...
		if($val['ebay_couny']){
			...
			$vv="select ebay_couny from ebay_order where ebay_countryname='$countryname' and (ebay_couny!='' and ebay_couny!='CN') and ebay_couny is not null limit 0,1";
			$vv = $dbcon->execute($vv);
			...
			if(count($vv) == 0 ){
				...
				$arrs['destinationCountryCode'] 	= $vv[0]['ebay_couny'];
			}
		...
		$arrs['customerWeight'] 			= number_format($goods_weight,2);
	...
	$result = $soap->createOrderService($request);
