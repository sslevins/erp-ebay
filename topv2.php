...
<?php if($user == 'survy' || $user == 'vip778' || $user == 'silent' || $user == 'viperp' || $user == 'vipbin'  || $user == 'vpcxbusps' || $user == 'vipfangni' || $user == 'vipwx' || $user == 'sz108' || $user == 'yml' || $user == 'CAN'  ){ ?>
<li class="notCurrentTabLeft"> 
...
<?php if($_SESSION['user'] == 'survy' || $_SESSION['user'] == 'viphong' || $_SESSION['user'] == 'yy2894' || $_SESSION['user'] == 'vip778' || $user == 'vipbin' || $user == 'vpcxbusps' || $user == 'vipnick' || $user == 'silent'  || $user == 'vipfangni' || $user == 'yml' || $user == 'CAN' ){ ?>
...
<?php
$sql	= "select count(ebay_id) as cc from ebay_order as a where a.ebay_status='1' and a.ebay_user='$user' and a.ebay_combine!='1'   and ($ebayacc)";
...
<?php
      //  if($ssname=='缺货'){
       ...
       // }
$sql	= "select count(ebay_id) as cc from ebay_order as a where a.ebay_status='$ssid'   and a.ebay_combine!='1'   and ($ebayacc)";
$sqla	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sqla);
echo $sql[0]['cc'];
?>
...
(<font color="#FF0000">
<?php
$sql		= "select ebay_id from ebay_order as a where ebay_user='$user' and ebay_status='2'  and ebay_combine!='1' and mailstatus='2' and ($ebayacc) ";
$query		= $dbcon->query($sql);
$total		= $dbcon->num_rows($query);
echo $total;
...
(<font color="#FF0000"><?php
$sql	= "select count(ebay_id) as cc from ebay_order where ebay_user='$user' and ebay_status='2' and profitstatus=1 ";
$sql	= $dbcon->execute($sql);
$sql	= $dbcon->getResultArray($sql);
echo $sql[0]['cc'];
?></font>)
