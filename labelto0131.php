...
$dbcon	= new DBClass();
	...
	for($i=0;$i<count($sql);$i++){
		...
		$strline .=  $ss[0]['carrier_sn'].';;;'.$ss[0]['username'].';'.$ss[0]['address'].';;'.$ss[0]['province'].';'.$ss[0]['city'].';'.$ss[0]['country'].';'.$ss[0]['tel'].';;'.$ss[0]['username'].';;';
		$strline .= $name.';'.$street1.' '.$street2.';'.$zip.';'.$state.';'.$city.';'.$countryname.';'.$tel.';;'.$ebay_userid.';'.$ebay_usermail.';1;;;;;;;';
		$values[$a][] = $strline;
		$a++;
}
$fp = fopen('images/ebay.csv', 'w');
 foreach($values as $k=>$v){
	 fputcsv($fp,$v);
 }
 fclose($fp);
 Header("Location:images/ebay.csv"); 
exit;
