...
$epagesize	= $_REQUEST['pagesize'];
	...
	$ss					= "select ebay_tracknumber,ebay_account,ebay_countryname,recordnumber from ebay_order where ($tjstr) and ebay_user ='$user' order by ebay_account,recordnumber ";
	$ss					= $dbcon->execute($ss);
	...
	for($i=0;$i<count($filenames);$i++){
			...
			echo 'Print- '.($i+1).'<a href="'.$filenames[$i].'" target ="_blank">Fiels-'.($i+1).'</a><br><br>';
	...
	*/
	...
	function getshiplabel($ordersn,$tkary){
		...
		$functions = $soapclient->GetHKShippingLabels(array("GetHKShippingLabelsRequest"=>$params));
