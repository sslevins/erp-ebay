...
<br />
			      ...
			      <tr>
					...
					for($i=0;$i<count($accounts);$i++){
							...
							/* 正常销售订单 */
							$ss		= "SELECT SUM( ebay_total ) AS total, ebay_currency FROM ebay_order where (ShippedTime>=$start1 and ShippedTime<=$end1) and ebay_account='$accountsname' and ebay_status='2' and profitstatus='1' and (ebay_ordertype is null or ebay_ordertype= '-1') GROUP BY ebay_currency";
							...
							/* 重寄销售订单 */
							$ss		= "SELECT SUM( ebay_total ) AS total, ebay_currency FROM ebay_order where (ShippedTime>=$start1 and ShippedTime<=$end1) and ebay_account='$accountsname' and ebay_status='2' and ebay_ordertype ='重寄订单'  GROUP BY ebay_currency";
							$ss		= $dbcon->execute($ss);
							$ss		= $dbcon->getResultArray($ss);
							$strline2	= '';
							for($t=0;$t<count($ss);$t++){
							...
							/* 成交费 */
							$ss		= "SELECT SUM(b.FinalValueFee) AS total FROM ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn = b.ebay_ordersn  where (a.ShippedTime>=$start1 and a.ShippedTime<=$end1) and a.ebay_account='$accountsname' and a.ebay_status='2' and profitstatus='1' ";
							...
							/* 计算paypal 手续费 */
							$ss		= "select sum(b.fee) as total,a.ebay_currency from ebay_order as a join ebay_paypaldetail as b on a.ebay_ptid = b.tid where (a.ShippedTime>=$start1 and a.ShippedTime<=$end1) and a.ebay_account='$accountsname' and a.ebay_status='2' and a.profitstatus='1'   GROUP BY a.ebay_currency";
							$ss		= $dbcon->execute($ss);
							$ss		= $dbcon->getResultArray($ss);
							...
							/* 计算总成本和总运费 */
							$ss		= "SELECT SUM( ordercopst ) AS ordercopst, SUM( ordershipfee ) AS ordershipfee FROM ebay_order where (ShippedTime>=$start1 and ShippedTime<=$end1) and ebay_account='$accountsname' and ebay_status='2'  and profitstatus='1'    ";
							$ss		= $dbcon->execute($ss);
							$ss		= $dbcon->getResultArray($ss);
							...
							$resendordercopst		= $ss[0]['ordercopst'] + $ss[0]['ordershipfee'];
                        ...
                        <tr>
						  ...
						  $ml			= $ebaytotal - $ebayfee - abs($pfee) - $ordercopst - $ordershipfee - $tkfee - $resendordercopst;
						  $mltotal		+= $ml;
					...
					//	  echo ' tkfee:'.$tkfee;
						...
						?>
                        <tr>
                          <td colspan="12"> <div style="border:#009900 dashed 1px"></div></td>
                          </tr>
  ...
  $totalline	= "Total AUD:{".number_format($aud,2)."} <br> Total CAD:{".number_format($cad,2)."} <br> Total GBP:{".number_format($gbp,2)."} <br> Total USD:{".number_format($usd,2)."} <br> Total EUR:{".number_format($eur,2)."}";						  
                          ...
                          <td><?php echo number_format($mltotal).' / '.number_format(($mltotal/$ttmb) * 100).'%' ?>&nbsp;</td>
                          <td>&nbsp;</td>
                        </tr>
