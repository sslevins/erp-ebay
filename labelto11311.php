...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '买家备注+我们自己的备注');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', '标题');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', '产品成本');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', '供应商');
	...
	for($i=0;$i<count($sql);$i++){
	    ...
	    $street1			= @$sql[$i]['ebay_note'].@$sql[$i]['ebay_noteb'];
		...
		for($o=0;$o<count($sl);$o++){			
			...
			$objPHPExcel->setActiveSheetIndex(0)->getCell('A'.$a)->setValueExplicit($ebay_id.chr(13).chr(13).chr(13), PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('B'.$a)->setValueExplicit($ebay_tracknumber, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('D'.$a)->setValueExplicit($sku1, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('E'.$a)->setValueExplicit($amount, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('F'.$a)->setValueExplicit($ebay_itemprice, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('G'.$a)->setValueExplicit($street1, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('H'.$a)->setValueExplicit($sku, PHPExcel_Cell_DataType::TYPE_STRING);
			
			
			
			
			
			$vv		= "select goods_cost,factory from ebay_goods where goods_sn='$sku1' and ebay_user = '$user'";	
			...
			$goods_cost		= $vv[0]['goods_cost'];
			$factory		= $vv[0]['factory'];	
			
			
			$vv 			= "select company_name from  ebay_partner where id='$factory' ";									
			$vv 			= $dbcon->execute($vv);
			$vv 			= $dbcon->getResultArray($vv);
			$company_name	= $vv[$i]['company_name'];
								
			$objPHPExcel->setActiveSheetIndex(0)->getCell('I'.$a)->setValueExplicit($goods_cost, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->getCell('J'.$a)->setValueExplicit($company_name, PHPExcel_Cell_DataType::TYPE_STRING);
