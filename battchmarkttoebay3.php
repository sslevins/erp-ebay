...
$dbcon	= new DBClass();
	...
	$vv				= "select distinct user  from ebay_user where   user = 'vipzz' or user ='silent' or user ='viphover' ";
	...
	for($j=0;$j<count($vv);$j++){
		...
		$ss			= "select ebay_account,ebay_id,ebay_ordersn,ebay_tracknumber,ebay_carrier,ebay_combine from ebay_order where ebay_user='$user' and ( ShippedTime ='' or ShippedTime is null )  and  ebay_status='2' and recordnumber > 0 and (ebay_carrier != 'EUB' or ebay_carrier is null) order by ebay_id desc limit 1000 ";
