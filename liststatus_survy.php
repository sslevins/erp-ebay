...
@session_start();
	...
	//error_reporting(E_ALL);	
		...
		$start0						= date('Y-m-d H:i:s',strtotime("$start1 -12 hours"));  // -8 
				...
				if(count($ss) >= 1){
							...
							$dbcon->execute($vv);							
								...
								if($newolqty > 0  && $track_stock == 0  && $totalitems[$i]['Variations']['Variation'] == '' ){
										...
										echo $i.': RUN: TO EBAY: '.$ItemID."<BR>\n\r";
								}else{										
										echo $i.': RUN: NO EBAY: '.$ItemID."<BR>\n\r";
								}
								
								$vv						= "update ebay_list set QuantityAvailable = '$QuantityAvailable',StartPrice='$StartPrice',QuantitySold='$QuantitySold' where id ='$id' ";
								$dbcon->execute($vv);
