...
$tj		= '';
	...
	for($i=0;$i<count($sql);$i++){
		...
		$ebay_id			= $sql[$i]['ebay_id'];
			...
			$vv = "select ebay_amount,sku,ebay_itemid from ebay_orderdetail where ebay_ordersn='".$sn."'";
			$vv = $dbcon->execute($vv);
			$vv = $dbcon->getResultArray($vv);
			
			
			$ebay_amount	= $vv[0]['ebay_amount'];
			
				$sku	= $vv[0]['sku'];
			
			
			
			$ss = "select goods_ywsbmc,goods_zysbmc,goods_sbjz,goods_weight,goods_unit,goods_width,goods_length,goods_height from ebay_goods where goods_sn='".$sku."' and ebay_user = '$user'";
			...
			$weight			= number_format($ss[0]['goods_weight'],3);
			$goods_zysbmc	= $ss[0]['goods_zysbmc'];
			$goods_ywsbmc	= $ss[0]['goods_ywsbmc'];
			$goods_sbjz		= number_format($ss[0]['goods_sbjz'],2);
				
					 
			
			
			$curlPost = 'username=rblkj&password=123456789&name='.$names.'&postcode='.$zip.'&phone='.$ebay_phone.'&mobile='.$ebay_phone.'&province='.$state.'&city='.$city.'&county='.$state.'&street='.$street1.'&cnname='.$goods_zysbmc.'&enname='.$goods_ywsbmc.'&count='.$ebay_amount.'&unit=pcs&weight='.$weight.'&delcarevalue='.$goods_sbjz.'&company='.$names;
...
echo $curlPost;
		...
		curl_setopt($ch,CURLOPT_URL,'http://www.shwise.cn:95/api.asp');//抓取指定网页
		...
		$t = XML_unserialize($data);
		$error_message		= $t['root']['error_message'];
		...
		if($status != '0' ){
			...
			echo '订单编号:'.$ebay_id.' 错误信息:'.$error_message.'<br>';
			...
			echo '订单编号:'.$ebay_id.' 成功申请<br>';
			
			$mailnum		= $t['root']['mailnum'];

			
			$usql		= "update ebay_order set ebay_tracknumber = '$mailnum' where ebay_id ='$ebay_id' and ebay_user ='$user'";
			$dbcon->execute($usql);
