...
while(true){
	...
	$ebay_userid				= str_rep(trim($currentSheet->getCell($ii)->getValue())); //买家名称
	$ebay_username				= str_rep(trim($currentSheet->getCell($ii)->getValue())); //买家名称
	...
	$ebay_paidtime				= trim($currentSheet->getCell($bb)->getValue()); // 付款时间
	...
	$ebay_shipfee				= str_rep(trim($currentSheet->getCell($dd)->getValue()));   // 物流费用
	$ebay_total					= str_rep(trim($currentSheet->getCell($cc)->getValue()));   //订单金额
	
	$productsinformation					= str_rep(trim($currentSheet->getCell('F'.$c)->getValue()));   //订单金额
	...
	for($j=0; $j<count($productsinformation);$j++){
			...
			if($labelstr != '' ){
				...
				$title		= str_replace('(specifics','',$title);
				
				$skul		= strripos($title,"(");
	
				
				$sku		= substr($title,$skul);
				$sku2		= strripos($sku,")");
				$sku		= substr($sku,1,$sku2-1);

				
			
				
				$qty		= $data[2];
				$qtylocation= strpos($qty,' ');
				$qty		= substr($qty,0,$qtylocation);
				$dataarray[$jj]['sku']	= $sku;
				
				$dataarray[$jj]['qty']	= $qty;
				$dataarray[$jj]['title']	= $title;
				$jj++;
	...
	$ebay_note					= str_rep(trim($currentSheet->getCell($gg)->getValue()));	//订单备注
	$ebay_state					= str_rep(trim($currentSheet->getCell($kk)->getValue()));
	$ebay_city					= str_rep(trim($currentSheet->getCell($ll)->getValue()));
	$ebay_street				= str_rep(trim($currentSheet->getCell($mm)->getValue()));
	$ebay_postcode				= str_rep(trim($currentSheet->getCell($nn)->getValue()));
	$ebay_phone					= str_rep(trim($currentSheet->getCell($oo)->getValue()));
	$ebay_carrier				= str_rep(trim($currentSheet->getCell($pp)->getValue()));   //买家选择物流
	
	$ebay_countryname			= str_rep(trim($currentSheet->getCell('J'.$c)->getValue()));
	...
	if($vv == '0'){
		...
		$sql					= "insert into  ebay_order(ebay_ordersn,ebay_paystatus,recordnumber,ebay_tid,ebay_ptid,ebay_orderid,ebay_createdtime,ebay_paidtime,ebay_userid,ebay_username,ebay_usermail,ebay_street,ebay_street1,ebay_city,ebay_state,ebay_countryname,ebay_postcode,ebay_phone,ebay_total,ebay_status,ebay_user,ebay_addtime,ebay_shipfee,ebay_account,ebay_note,ebay_carrier,ebay_warehouse) values('$val','Complete','$recordnumber','$ebay_tid','$ebay_ptid','$ebay_orderid','$ebay_createdtime','$ebay_paidtime','$ebay_userid','$ebay_username','$ebay_usermail','$ebay_street','$ebay_street1','$ebay_city','$ebay_state','$ebay_countryname','$ebay_postcode','$ebay_phone','$ebay_total','$orderstatus','$user','$mctime','$ebay_shipfee','$account','$ebay_note','$ebay_carrier','$defaultstoreid')";
