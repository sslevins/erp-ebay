...
$country['turkey']					=	'土耳其';
	...
	$country['Zimbabwe']				=	'津巴布韦';
		...
		$ertj		= "";
			...
			$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where a.ebay_userid !=''  and ($ertj) group by a.ebay_id order by b.goods_location,b.sku ";
...
<tr>
              <td><img src="barcode128.class.php?data=<?php echo $ebay_id; ?>" alt="" width="164" height="33"/></td>
          <td><?php echo $ebay_id; ?>&nbsp;<?php echo ($i+1).'/'.$totalpages; ?></td>
          </tr>
<tr>
  <td colspan="2"><?php echo $addressline;?>&nbsp;</td>
  </tr>
            <tr>
              <td colspan="2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="word-break:break-all;"><tr><td width="89%" rowspan="2" align="left" valign="bottom">
                
                
                <?php
				
				
				 $bb		= "select sku,ebay_amount,attribute from ebay_orderdetail where ebay_ordersn ='$ordersn'";
		 $gg		= $dbcon->execute($bb);
		 ...
		 for($t=0;$t<count($gg);$t++){
			...
			$attribute		= $gg[$t]['attribute'];
			$totalqty		= $totalqty + $ebay_amount;
			
			
			$ee					= "SELECT goods_location FROM ebay_goods where goods_sn='$sku' and ebay_user='$user'";
			$ee					= $dbcon->execute($ee);
			...
			if(count($rr) > 0){
									...
									for($e=0;$e<count($goods_sncombine);$e++){
											...
											$ee 	 	= $dbcon->getResultArray($ee);
											$goods_location		=  $ee[0]['goods_location'];
											echo $goods_sn.' '.$attribute. '*'.$goddscount.' &nbsp;&nbsp;&nbsp;';
											$tt++;
											
											if(($tt) % 2 == 0 && count($gg) != $tt) echo '<br>';
				...
				echo $sku.' '.$attribute. '*'.$ebay_amount.' &nbsp;&nbsp;&nbsp;';
				$tt++;
				if(($tt) % 2 == 0 && count($gg) != $tt) echo '<br>';
			}
            ...
            ?>
                ...
                &nbsp;</td>
                    <td width="11%" rowspan="2" align="right" valign="bottom"></td>
                </tr>
              </table>
</td>
            </tr>
