<?php
	...
	$country['Zimbabwe']				=	'津巴布韦';
		...
		$ertj		= "";
			...
			$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where  ($ertj) group by a.ebay_id order by  b.sku ";
			
			}else{
			
			$sql			= "select a.*,b.sku from ebay_order as a join ebay_orderdetail as b on a.ebay_ordersn=b.ebay_ordersn where ebay_status='$ostatus' and ebay_combine!='1' and ($ebayacc) group by a.ebay_id order by b.sku ";
...
//idPrint3.disabled = false; 
   ...
   $pages	= 1;
 	...
 	for($i=0;$i<count($sql);$i++){
				...
				$ss					= "select address from ebay_carrier where name='$ebay_carrier'";			
				$ss					= $dbcon->execute($ss);
				$ss					= $dbcon->getResultArray($ss);
				$backaddress			= $ss[0]['address'];
				
				
				if($user == 'vipzz'){
				
				$backaddress = 'From：Jiangting (C8750)<br>
Lutong Shipping Limited P.O. Box No.<br> 68600 Kowloon East Post Office,HongKong ';
				}
				...
				if($street2 != ''){
					...
					$addressline	= $cname."<br>".$street1."<br>".$street2."<br>".$city.",".$state.",".$zip."<br>".$countryname."(".$country[$countryname].")";
					
				
				}else{
					
					$addressline	= $cname."<br>".$street1."<br>".$city.", ".$state.",".$zip."<br>".$countryname."(".$country[$countryname].")";
  ...
  ?>
<table width="712" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000" style="width:190mm; height:52mm; text-align: right;">
  <tr>   
    <td align="center" valign="top" style='width:92mm'><table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding:6px">
      
      
      
      <tr>
        <td width="100%" class="STYLE18"><span style="font-size:12; padding-bottom:5px;"><?php echo $backaddress;?></span>
		<div align="left" style="margin:0px 0px 5px 0px; font-family:Arial, Helvetica, sans-serif; float:left;"></div><div style="margin:0px 10px 1px 0px; font-family:Arial, Helvetica, sans-serif; float:right;"><strong><?php echo $recordnumber;?></strong></div>		</td>
	  </tr>
	  
	  <tr>
        <td width="100%" valign="top" class="STYLE18">
         <div style='font-family:Arial, Helvetica, sans-serif; position: relative; line-height:20px;'>
			<strong><?php echo 'Send TO: '.$addressline;?></strong>
			<div style='width:101px; height:38px;position:absolute; right:0px; bottom:5px;'>
				<img src="barcode128.class.php?data=<?php echo $ebayid; ?>" alt="" width="100" height="35"/>
				<div style='width:100%; height:10px; margin-top:-3px' align="center"><?php echo $ebayid;?></div>
		  </div>		</td>
      </tr>
      
      
    </table></td>
    <td align="center" valign="top" style='92mm'><table width="100%" border="0" cellspacing="1" cellpadding="3">
        
        <tr>
          <td class="STYLE18"><div align="left" style="font-family:Arial, Helvetica, sans-serif">
            <?php
		  	$sl	= "select sku,ebay_amount,ebay_itemurl from ebay_orderdetail where ebay_ordersn='$ordersn' ";
		$sl	= $dbcon->execute($sl);
		...
		for($o=0;$o<count($sl);$o++){
			$sku	= $sl[$o]['sku'];
			$ebay_itemurl 	= $sl[$o]['ebay_itemurl'];
			...
			if(count($rr) > 0){	
				...
				for($e=0;$e<count($goods_sncombine);$e++){
					...
					$goods_sn		= $pline[0];
					$goodscount     = $pline[1] * $amount;
					$yy			= "select goods_name,goods_pic from ebay_goods where goods_sn='$goods_sn' and ebay_user ='$user'";
					$yy			= $dbcon->execute($yy);
					$yy			= $dbcon->getResultArray($yy);
					$goods_name	= $yy[0]['goods_name'];
					$goods_pic	= $yy[0]['goods_pic']?$yy[0]['goods_pic']:$goods_sn.'.jpg';
					$a = $a+$goodscount;
					if($q<=3){
						echo "<div style='float:left;width:28mm'><img src='images/".$goods_pic."' style='width:20mm;height:20mm;margin:5px'><div>".$goods_sn."[".$goods_name."]*".$goodscount."</div></div>";
						$q++;
					}					
				}			
			}else{
				$yy			= "select goods_name,goods_pic from ebay_goods where goods_sn='$sku' and ebay_user ='$user'";
				$yy			= $dbcon->execute($yy);
				$yy			= $dbcon->getResultArray($yy);
				$goods_name	= $yy[0]['goods_name'];
				$goods_pic	= $yy[0]['goods_pic']?$yy[0]['goods_pic']:$sku.'.jpg';
				$a = $a+$amount;
				if($q<=3){
					echo "<div style='float:left;width:28mm'><img src='".$ebay_itemurl ."' style='width:20mm;height:20mm;margin:5px'><div>".$sku."[".$goods_name."]*".$amount."</div></div>";
					$q++;
				}					
			}
		}
        ...
        <tr>
          <td class="STYLE18">
			<?php echo "<br>&nbsp;<strong>Record:".$recordnumber.' '.$ebayaccount."总数：$a </strong>BuyerID:".$userid."<br>"?>			</td>
        </tr>
    </table></td>
</tr>
</table>
