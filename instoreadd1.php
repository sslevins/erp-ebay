...
include "top.php";	
	...
	function addoroutstock($io_ordersn,$type,$storeid){
		...
		for($i=0;$i<count($sql);$i++){
			...
			if(count($seq) == 0){
				...
				$sq			= "insert into ebay_onhandle(goods_id,goods_count,store_id,ebay_user) values('$goods_id','$goods_count','$storeid','$user')";
				if($dbcon->execute($sq)){
	...
	if($_REQUEST['addtype'] == 'save'){
		...
		if(count($sql) == 0){
			...
			$sql	= "update ebay_iostore set io_warehouse='$in_warehouse',io_type='$in_type',io_note='$note',partner='$partner' where io_ordersn='$io_ordersn'";
...
<tr>
					...
					$sql	 = "SELECT * FROM `ebay_storetype` where ebay_user='$user' and ebay_storetype='0'";
					$sql	 = $dbcon->execute($sql);
					$sql	 = $dbcon->getResultArray($sql);
					for($i=0;$i<count($sql);$i++){					
					...
					for($i=0;$i<count($sql);$i++){					
					 	...
					 	$company_name	= $sql[$i]['company_name']."   ".$sql[$i]['username'];
						$cid			=  $sql[$i]['id'];
					 ?>
                            <option value="<?php echo $cid;?>" <?php if($partner == $cid) echo "selected=selected" ?>><?php echo $company_name;?></option>
                            <?php } ?>
                          </select>&nbsp;</td>
                          <td>&nbsp;</td>
                          <td>&nbsp;</td>
...
<script language="javascript">
	...
	function del(id){
		...
		location.href="instoreadd.php?&module=orders&action=入库&io_ordersn=<?php echo $io_ordersn;?>&addtype=del&stype=<?php echo $stype;?>&id="+id;
	...
	function add(){
		...
		location.href="instoreadd.php?&module=warehouse&action=入库&io_ordersn=<?php echo $io_ordersn;?>&goods_sn="+goods_sn+"&goods_count="+goods_count+"&addtype=add&stype=<?php echo $stype;?>";
	...
	function save(){
		...
		var url					= "instoreadd.php?addtype=save&in_type="+in_type+"&in_warehouse="+in_warehouse+"&note="+note+"&module=warehouse&action=入库&io_ordersn=<?php echo $io_ordersn;?>&stype=<?php echo $stype;?>&partner="+partner;
		location.href			= url;
	...
	function audit(){
		...
		var url					= "instoreadd.php?addtype=audit&astatus=1&action=入库&io_ordersn=<?php echo $io_ordersn;?>&module=warehouse&action=入库&stype=<?php echo $stype;?>";
		location.href			= url;
	...
	function auditf(){
		...
		var url					= "instoreadd.php?addtype=audit&astatus=0&action=入库&io_ordersn=<?php echo $io_ordersn;?>&module=warehouse&action=入库&stype=<?php echo $stype;?>";
		location.href			= url;
