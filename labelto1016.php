...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '当前库存数量');
...
where a.ebay_user='$user'   and a.ebay_combine!='1' and ($ertj) GROUP BY b.sku ";	
	...
	for($i=0;$i<count($sql);$i++){
		...
		$sku					= $sql[$i]['sku'];	
			...
			if(count($rr) > 0){
				...
				$vv 		 	= $dbcon->getResultArray($vv);
				if(count($vv) >0){
						$vv			= "update ebay_labeltotable set goods_amount = goods_amount + $amount where goods_sn ='$sku' ";
				}else{
						$vv			= "insert into ebay_labeltotable (goods_sn,goods_name,goods_amount) values('$sku','$goods_name','$amount') ";
				}
				$dbcon->execute($vv);
	...
	for($i=0;$i<count($vv);$i++){
				...
				$ff				= "select * from ebay_onhandle where goods_sn ='$sku' and ebay_user ='$user' ";
				
				$ff				= $dbcon->execute($ff);
				$ff 		 	= $dbcon->getResultArray($ff);
				
				$goods_count	= $ff[0]['goods_count'];
		
				if($goods_amount > $goods_count ){
				...
				$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$a, $goods_count, PHPExcel_Cell_DataType::TYPE_STRING);
				$a++;
