...
<?PHP
	...
	function eubtracknumber($ordersn){
		...
		"OrderDetail"=> array("PickUpAddress"=>$PickUpAddress,"ShipFromAddress"=>$ShipFromAddress,"ShipToAddress"=>$ShipToAddress,"ItemList"=>array("Item"=>$item),"EMSPickUpType"=>$shiptype));
		...
		foreach($functions as $aa){
			...
			if($ack     != 'Failure'){
				...
				$sg			= "update ebay_order set ebay_tracknumber='$TrackCode', ebay_carrier='EUB' where ebay_id = '$ordersn'";
				$dbcon->execute($sg);
