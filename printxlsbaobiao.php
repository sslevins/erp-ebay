...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '订单号-内');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '订单号-外');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', '买家ID/E-mail');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', '挂号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', '寄件人姓名');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', '寄件人地址');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '寄件人电话');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', '收货人');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', '地址');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', '收件人电话');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', '国家');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1', '邮件种类');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M1', '品名');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N1', '数量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O1', '申报价值');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('P1', '重量');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q1', '运送方式');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R1', '销售额');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('S1', '产品成本');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('T1', '运费');
	...
	$sql	= "select * from ebay_order as a where ebay_user='$user'  and a.ebay_combine!='1' and (a.ebay_addtime  >=$start and a.ebay_addtime  <=$end) ";	
	
	if($ebay_carrier != '') 		$sql .= " and ebay_carrier ='$ebay_carrier' ";
	if($_REQUEST['account'] != '' ) $sql .= " and ($tjstr) ";
	...
	for($i=0;$i<count($sql);$i++){
		...
		$packinguser			= $sql[$i]['packagingstaff'];
		$ordershipfee			= $sql[$i]['ordershipfee'];
		$ebay_id				= $sql[$i]['ebay_id'];
		$orderweight2				= $sql[$i]['orderweight2'];	
		$ordershipfee				= $sql[$i]['ordershipfee'];	
		
		$vv			= "select sum(ebay_amount) as cc from ebay_orderdetail where ebay_ordersn = '$ordersn'";
		$vv			= $dbcon->execute($vv);
		$vv			= $dbcon->getResultArray($vv);
		$amount		= $vv[0]['cc'];
		
		/* 取得产品的成本 */
		$vv			= "select sku from ebay_orderdetail where ebay_ordersn = '$ordersn'";
		$vv			= $dbcon->execute($vv);
		$vv			= $dbcon->getResultArray($vv);
		$goods_cost = 0;
		...
		for($v=0;$v<count($vv);$v++){
			...
			if(count($rr) > 0){
									...
									for($e=0;$e<count($goods_sncombine);$e++){
											...
											$ee				= "SELECT goods_cost FROM ebay_goods where goods_sn='$goods_sn' and ebay_user='$user'";
											$ee				= $dbcon->execute($ee);
											$ee 	 		= $dbcon->getResultArray($ee);
											$goods_cost		+=  $ee[0]['goods_cost'];	
									}
									
			}else{
			
			
											$ee				= "SELECT goods_cost FROM ebay_goods where goods_sn='$sku' and ebay_user='$user'";
											$ee				= $dbcon->execute($ee);
											$ee 	 		= $dbcon->getResultArray($ee);
											$goods_cost		+=  $ee[0]['goods_cost'];	
			}
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$a, $ebay_id);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$a, $recordnumber0);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$a, $ebay_userid);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$a, $ebay_tracknumber);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$a, '陈阳波');
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$a, '深圳坂田');
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$a, '84717419');
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$a, $name);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$a, $street1.' '.$street2);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$a, $ebay_phone);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K'.$a, $cnname);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('P'.$a, $orderweight2);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$a, $ebay_carrier);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$a, $ebay_total);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$a, $amount);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('R'.$a, $ebay_total);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('S'.$a, $goods_cost);
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('T'.$a, $ordershipfee);
