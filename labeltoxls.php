...
$objPHPExcel->getProperties()->setCreator("Maarten Balliauw")
	...
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', '客户ID');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1', '国家');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1', 'CASE日期');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1', '店名');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1', 'CASE类型');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1', 'CASE原因');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1', '原定单号');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', 'SKU');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', '描述');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', '退款金额');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1', '操作备注');
	
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1', '退款类型');
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('M1', '描述');
	...
	if($_REQUEST['bill'] == ''){
	$sql	= "select * from ebay_rma as a where id > 0 ";
	}else{
	$sql	= "select * from ebay_rma as a where ($ertj) ";
	}
	...
	for($i=0;$i<count($sql);$i++){
			...
			$sku							= $sql[$i]['sku'];
			$pid							= $sql[$i]['ebay_pid'];
			$REFUNDTYPE							= $sql[$i]['REFUNDTYPE'];
			
			if($REFUNDTYPE == 'Full') $REFUNDTYPE = '全额退款';
			if($REFUNDTYPE == 'Partial') $REFUNDTYPE = '部分退款';
			
			
			
			$Description							= $sql[$i]['Description'];
			
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A'.$a, $userid, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('B'.$a, $countrys, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$a, $OpenDate, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('D'.$a, $ebay_account, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('E'.$a, $rtatype, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('F'.$a, $status, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('G'.$a, $ebay_id	, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('H'.$a, $sku	, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('L'.$a, $REFUNDTYPE	, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('M'.$a, $Description	, PHPExcel_Cell_DataType::TYPE_STRING);
			
			
			$ebay_refundamount						= $sql[$i]['ebay_refundamount'];
			$Description						= $sql[$i]['Description'];
						$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('I'.$a, $Description	, PHPExcel_Cell_DataType::TYPE_STRING);
			$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('J'.$a, $ebay_refundamount	, PHPExcel_Cell_DataType::TYPE_STRING);

			
			
			
						$ss	= "select * from ebay_rmaactions where ebay_id='$ebay_id' and ebay_pid='$pid'";
						$ss = $dbcon->execute($ss);
						...
						for($g=0;$g<count($ss);$g++){
								...
								$content		= $ss[$g]['content'];
								$wuser			= $ss[$g]['wuser'];
								$tdate			= $ss[$g]['tdate'];
								
								$strstr			.= $content.' -- '.$wuser.' -- '.$tdate.chr(13);
								
								
								}
								
									$objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('K'.$a, $strstr	, PHPExcel_Cell_DataType::TYPE_STRING);
...
$title		= "Case".date('Y-m-d');
$titlename		= "Case".date('Y-m-d').".xls";
